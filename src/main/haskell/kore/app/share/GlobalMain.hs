...
globalCommandLineParser =
    GlobalOptions <$>
    flag False True (long "version" <> help "Print version information")
...
commandLineParse localCommandLineParser modifiers =
    execParser $
    info
        (MainOptions <$> globalCommandLineParser <*>
         (Just <$> localCommandLineParser <|> pure Nothing) <**>
         helper)
        modifiers
...
enableDisableFlag name enabledVal disabledVal defaultVal helpSuffix =
    flag' enabledVal (hidden <> internal <> long name <> help helpSuffix) <|>
    flag'
        disabledVal
        (hidden <> internal <> long ("no-" ++ name) <> help helpSuffix) <|>
    flag'
        disabledVal
        (long ("[no-]" ++ name) <> help ("Enable/disable " ++ helpSuffix)) <|>
    pure defaultVal
