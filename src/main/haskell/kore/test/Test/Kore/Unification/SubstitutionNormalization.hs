...
import Kore.AST.Common (AstLocation(..), Variable)
...
import Kore.Variables.Fresh.IntCounter (runIntCounter)
...
test_substitutionNormalization =
    ...
    , testCase
          "Simple unnormalized substitution"
          (assertEqual
               ""
               (Right
                    [ ( asVariable (v1 PatternSort)
    ...
    , testCase
          "Unnormalized substitution with 'and'"
          (assertEqual
               ""
               (Right
                    [ ( asVariable (v1 PatternSort)
    ...
    , let var1 = asVariable (v1 PatternSort)
       in testCase
              "Simplest cycle"
              (assertEqual
                   ""
                   ...
                   (runNormalizeSubstitution
                        [(var1, asPureMetaPattern (v1 PatternSort))]))
    , let var1 = asVariable (v1 PatternSort)
       ...
       in testCase
              ...
              (assertEqual
                   ...
                   (runNormalizeSubstitution
                        [ (var1, asPureMetaPattern (x1 PatternSort))
                        , (varx1, asPureMetaPattern (v1 PatternSort))
                        ]))
    , let var1 = asVariable (v1 PatternSort)
          varx1 = asVariable (x1 PatternSort)
       in testCase
...
runNormalizeSubstitution ::
    ...
    -> Either (SubstitutionError level Variable) (UnificationSubstitution level Variable)
