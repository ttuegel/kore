...
import Kore.ASTUtils.SmartConstructors (mkVar)
...
import Test.Kore
import Test.Kore.AST.MLPatterns (extractPurePattern)
...
symbols =
    map (\s -> (getSentenceSymbolOrAliasHead s [], s))
        [ a
        , a1
        , a2
        , a3
...
unificationSubstitution ::
       Substitution Object -> [(Variable Object, CommonPurePattern Object)]
unificationSubstitution = map trans
  where
    trans (v, p) =
        let pp = extractPurePattern p
         in ( Variable
                  { variableSort =
...
unificationResult (UnificationResultTerm pat) sub =
    UnificationSolution
    ...
    subst'' =
        subst'
            { unificationSolutionConstraints =
                  sortBy
                      (compare `on` fst)
                      (unificationSolutionConstraints subst')
            }
...
unificationProcedureSuccess ::
    ...
    -> UnificationProof Object Variable
    -> TestTree
unificationProcedureSuccess message (UnificationTerm term1) (UnificationTerm term2) subst predicate' proof =
    testCase
        message
        (assertEqualWithExplanation
             ""
...
test_unification =
    ...
    , andSimplifySuccess
          "equal non-constructor patterns"
          (UnificationTerm a2A)
          (UnificationTerm a2A)
    ...
    , andSimplifySuccess
          "variable + non-constructor pattern"
          (UnificationTerm a2A)
          (UnificationTerm x)
          (UnificationResultTerm a2A)
          [("x", a2A)]
    ...
    , andSimplifySuccess
          ...
          [("ex1", applyS eg [ex3]), ("ex2", ex3), ("ex4", applyS eh [ex1])]
          (AndDistributionAndConstraintLifting
    ...
    , andSimplifySuccess
          ...
          (UnificationTerm (applyS nonLinF [applyS nonLinG [nonLinX], nonLinX]))
    ...
    , andSimplifySuccess
          ...
          (UnificationResultTerm
               (applyS
                    expBin
                    [applyS expBin [expA, expY], applyS expBin [expY, expA]]))
          [("x", applyS expBin [expA, expY]), ("x", applyS expBin [expY, expA])]
          (AndDistributionAndConstraintLifting
               (symbolHead expBin)
               [ Proposition_5_24_3
    ...
    , unificationProcedureSuccess
          ...
          [("a", applyS eg [expX]), ("x", applyS eg [expX])]
          makeTruePredicate
    ...
    , unificationProcedureSuccess
          ...
          (UnificationTerm (applyS expBin [applyS expBin [expA, expY], expX]))
          (UnificationTerm (applyS expBin [expX, applyS expBin [expY, expA]]))
          [("a", expY), ("x", applyS expBin [expY, expA])]
          makeTruePredicate
          (CombinedUnificationProof
               [ AndDistributionAndConstraintLifting
                     (symbolHead expBin)
               ...
               , ConjunctionIdempotency (extractPurePattern expY)
               ])
    , andSimplifyFailure
    ...
    , andSimplifyFailure
          "non-constructor symbolHead right"
          (UnificationTerm aA)
          (UnificationTerm a2A)
          (NonConstructorHead (symbolHead a2))
    , andSimplifyFailure
          "non-constructor symbolHead left"
          (UnificationTerm a2A)
          (UnificationTerm aA)
          (NonConstructorHead (symbolHead a2))
    , andSimplifyFailure
    ...
    , testCase
          ...
          (assertEqualWithExplanation
               ""
               [(W "1", war' "2")]
               (mapSubstitutionVariables showVar [(V 1, var' 2)]))
    ]
  where
    symbolHead symbol = getSentenceSymbolOrAliasHead symbol []
    var ps =
        case project (extractPurePattern ps) of
            VariablePattern v -> v
