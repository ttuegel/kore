...
data TestConfiguration level = TestConfiguration
    ...
    , testConfigurationCaseBasedConfiguration :: ![( [TestFlag]
                                                   , AdditionalTestConfiguration)]
    }
...
testsForObjectSort ::
    ...
    -> NamePrefix
    -> TestTree
testsForObjectSort (CommonDescription commonDescription) successConfiguration failureConfiguration expectedErrorMessage errorStack sort namePrefix@(NamePrefix rawNamePrefix) =
    testGroup
        commonDescription
...
addSentenceToTestConfiguration sentence configuration@TestConfiguration {testConfigurationAdditionalSentences = existingSentences} =
    configuration
...
successTestsForMetaSort ::
    ...
    -> NamePrefix
    -> TestTree
successTestsForMetaSort (CommonDescription commonDescription) successConfiguration@(SuccessConfiguration testConfiguration) sort additionalSortActual namePrefix =
    testGroup
        commonDescription
  ...
  where
    flaggedTests =
        flaggedMetaTestsForSort
            ...
            namePrefix
successTestsForMetaSort (CommonDescription commonDescription) SuccessConfigurationSkipAll _ _ _ =
...
failureTestsForMetaSort ::
    ...
    -> NamePrefix
    -> TestTree
failureTestsForMetaSort (CommonDescription commonDescription) failureConfiguration@(FailureConfiguration testConfiguration) expectedErrorMessage errorStack sort additionalSortActual namePrefix =
    testGroup
        commonDescription
  ...
  where
    flaggedTests =
        flaggedMetaTestsForSort
            ...
            additionalSortActual
            namePrefix
failureTestsForMetaSort (CommonDescription commonDescription) FailureConfigurationSkipAll _ _ _ _ _ =
    testGroup commonDescription []
...
expectFailureWithErrorFlaggedTests ::
    ...
    -> [FlaggedTestData]
    -> TestTree
expectFailureWithErrorFlaggedTests (FailureConfiguration testConfiguration) errorMessage additionalErrorStack flaggedTests =
    testGroup
        (testConfigurationDescription testConfiguration)
...
newtype TestedSort level =
    TestedSort (Sort level)
...
unfilteredTestExamplesForSort ::
    ...
    -> [FlaggedTestData]
unfilteredTestExamplesForSort x (TestedSort sort) (SortActualThatIsDeclared additionalSortActual) sortVariables (NamePrefix identifierPrefix) sentenceAliasSentence sentenceSymbolSentence =
    [ FlaggedTestData
...
unfilteredTestExamplesForObjectSort ::
    ...
    -> NamePrefix
    -> [FlaggedTestData]
unfilteredTestExamplesForObjectSort (TestedSort sort) (SortActualThatIsDeclared additionalSortActual) sortVariables (NamePrefix namePrefix) =
    [ FlaggedTestData
          { flaggedTestDataFlags = []
          , flaggedTestDataTestData =
                \additionalSentences ->
