...
failureTestData (ExpectedErrorMessage message) (ErrorStack stack) testData =
    expectFailureWithError
        ...
        err {errorError = message, errorContext = errorContext err ++ stack}
...
newtype OperandSort level =
    OperandSort (Sort level)

newtype ResultSort level =
    ResultSort (Sort level)

newtype DeclaredSort level =
    DeclaredSort (Sort level)

newtype TestedPatternSort level =
    TestedPatternSort (Sort level)
...
simpleMetaAliasSentence alias sort =
    asSentence (simpleAliasSentence alias sort l r :: KoreSentenceAlias Meta)
  where
...
simpleAliasSentence (AliasName name) (SortName sort) l r =
    SentenceAlias
        { sentenceAliasAlias =
              Alias {aliasConstructor = testId name, aliasParams = []}
        , sentenceAliasSorts = []
        , sentenceAliasResultSort =
              SortActualSort
                  SortActual
                      {sortActualName = testId sort, sortActualSorts = []}
...
simpleSymbolSentence (SymbolName name) (SortName sort) =
    SentenceSymbol
        { sentenceSymbolSymbol =
              Symbol {symbolConstructor = testId name, symbolParams = []}
        , sentenceSymbolSorts = []
...
aliasSentenceWithSort :: AliasName -> Sort Meta -> KoreSentence
...
aliasSentenceWithSortParameters ::
    ...
    -> Pattern level Variable (Fix (UnifiedPattern Variable))
    -> KoreSentenceAlias level
aliasSentenceWithSortParameters (AliasName name) (SortName sort) parameters l r =
    SentenceAlias
        { sentenceAliasAlias =
              Alias {aliasConstructor = testId name, aliasParams = parameters}
        , sentenceAliasSorts = []
        , sentenceAliasResultSort =
              SortActualSort
                  SortActual
...
sentenceAliasWithSortArgument ::
    ...
    -> Pattern level Variable (Fix (UnifiedPattern Variable))
    -> KoreSentenceAlias level
sentenceAliasWithSortArgument (AliasName name) sortArgument resultSort parameters l r =
    SentenceAlias
        { sentenceAliasAlias =
              Alias {aliasConstructor = testId name, aliasParams = parameters}
...
sentenceAliasWithAttributes (AliasName name) params sort attributes l r =
    SentenceAlias
        { sentenceAliasAlias =
              Alias {aliasConstructor = testId name, aliasParams = params}
        , sentenceAliasSorts = []
        , sentenceAliasResultSort = sort
        , sentenceAliasLeftPattern = l
        , sentenceAliasRightPattern = r
...
sentenceSymbolWithAttributes (SymbolName name) params sort attributes =
    SentenceSymbol
        { sentenceSymbolSymbol =
              Symbol {symbolConstructor = testId name, symbolParams = params}
        , sentenceSymbolSorts = []
...
symbolSentenceWithSortParameters (SymbolName name) (SortName sort) parameters =
    SentenceSymbol
        { sentenceSymbolSymbol =
              Symbol
                  {symbolConstructor = testId name, symbolParams = parameters}
        , sentenceSymbolSorts = []
        , sentenceSymbolResultSort =
              SortActualSort
                  SortActual
...
sentenceAliasWithResultSort ::
    ...
    -> Pattern level Variable (Fix (UnifiedPattern Variable))
    -> KoreSentenceAlias level
sentenceAliasWithResultSort (AliasName name) sort parameters l r =
    SentenceAlias
        { sentenceAliasAlias =
              Alias {aliasConstructor = testId name, aliasParams = parameters}
        , sentenceAliasSorts = []
...
aliasSentenceWithArguments ::
    ...
    -> Pattern level Variable (Fix (UnifiedPattern Variable))
    -> KoreSentence
aliasSentenceWithArguments (AliasName name) sort operandSorts l r =
    constructUnifiedSentence SentenceAliasSentence $
    SentenceAlias
        { sentenceAliasAlias =
              Alias {aliasConstructor = testId name, aliasParams = []}
...
simpleSortActual (SortName sort) =
    SortActual {sortActualName = testId sort, sortActualSorts = []}
...
stringUnifiedPattern s = asKorePattern (StringLiteralPattern (StringLiteral s))

variable :: VariableName -> Sort level -> Variable level
variable (VariableName name) sort =
    Variable {variableName = testId name, variableSort = sort}

unifiedVariable ::
       MetaOrObject level => VariableName -> Sort level -> Unified Variable
...
unifiedVariablePattern name sort = asKorePattern (variablePattern name sort)
...
simpleExistsUnifiedPattern name sort =
    asKorePattern (simpleExistsPattern (variable name sort) sort)
...
simpleExistsEqualsUnifiedPattern ::
    ...
    -> ResultSort level
    -> CommonKorePattern
simpleExistsEqualsUnifiedPattern name (OperandSort operandSort) (ResultSort resultSort) =
    asKorePattern
        (ExistsPattern
             Exists
                 { existsSort = resultSort
...
applicationObjectUnifiedPatternWithChildren name unifiedPatterns =
    asKorePattern
        (applicationPatternWithChildren name unifiedPatterns :: Pattern Meta Variable CommonKorePattern)
