...
test_withSort =
    testGroup
        ...
        , testCase
              "withSort keeps the same sort sort"
              (assertSortedStub
                   SortedPattern
                       { sortedPatternPattern =
                             TopPattern Top {topSort = sortListMetaSort}
    ...
    assertSortedStub expectedSorted stub =
        case stub of
            ...
            SortedPatternStub sorted ->
                assertEqual
                    "Expecting a pattern with the given sort"
                    expectedSorted
                    sorted
