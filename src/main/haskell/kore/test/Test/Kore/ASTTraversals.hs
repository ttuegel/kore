module Test.Kore.ASTTraversals
    ( test_astTraversals
    ) where
...
test_astTraversals =
    [ testCase
          ...
          (assertEqual "" samplePatternExpected (lhs samplePattern))
    , testCase
          "Testing bottomUpVisitorM"
          (assertEqual "" samplePatternExpected (lhs2 samplePattern))
    ]
  where
                      ...
                      [ asKorePattern $
                        ImpliesPattern
                            Implies
                                { impliesSort =
                                      SortVariableSort $
                                      SortVariable $ testId "#a" :: Sort Meta
                                , impliesFirst =
                                      asKorePattern $
                      ...
                      , asKorePattern $
                        ImpliesPattern
                            Implies
                                { impliesSort =
                                      SortVariableSort $
                                      SortVariable $ testId "#b" :: Sort Meta
                                , impliesFirst =
                                      asKorePattern $
                                      StringLiteralPattern
                                          (StringLiteral "left2")
