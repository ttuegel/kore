...
import Kore.AST.PureML (CommonPurePattern, fromPurePattern)
...
import Test.Kore.Step.Condition (mockConditionEvaluator)
...
test_userDefinedFunction =
    [ testCase
          "Cannot apply function if step fails"
    ...
    , testCase
          "Cannot apply step with unsat condition"
          (assertEqualWithExplanation
               ""
               ...
               (evaluateWithAxiom
                    ...
                    (asApplication (metaF (x PatternSort)))))
    , testCase
          "Reevaluates the step application"
          (assertEqualWithExplanation
               "f(x) => g(x) and g(x) => h(x)"
               (AttemptedFunction.Applied
                    ExpandedPattern
               ...
               (evaluateWithAxiom
                    ...
                    (asApplication (metaF (x PatternSort)))))
    , testCase
          "Does not reevaluate the step application with incompatible condition"
          (assertEqualWithExplanation
               ...
               (evaluateWithAxiom
                    ...
                    (asApplication (metaF (x PatternSort)))))
    , testCase
          "Preserves step substitution"
          (assertEqualWithExplanation
               "sigma(x,x) => g(x) vs sigma(a, b)"
               (AttemptedFunction.Applied
                    ExpandedPattern
    ...
    , testCase
          "Merges the step substitution with the reevaluation one"
          (assertEqualWithExplanation
...
evaluateWithAxiom metadataTools axiom conditionEvaluator functionEvaluator app =
    fst $
    fst $
    runIntCounter
        (give
