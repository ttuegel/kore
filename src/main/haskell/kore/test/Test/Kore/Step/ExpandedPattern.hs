module Test.Kore.Step.ExpandedPattern
...
import Kore.ASTUtils.SmartConstructors (mkAnd, mkBottom, mkEquals, mkTop, mkVar)
...
test_expandedPattern =
    ...
    , testCase
          ...
          (assertEqualWithExplanation
               ""
               (makeAnd
                    (makeAnd (var 1) (makeEq (var 2) (var 3)))
                    (makeEq (var 4) (var 5)))
    ...
    , testCase
          ...
          (assertEqualWithExplanation
               ""
               (makeAnd (makeEq (var 2) (var 3)) (makeEq (var 4) (var 5)))
               (give mockMetadataTools $
                ExpandedPattern.toMLPattern
                    ExpandedPattern
                        { term = mkTop
...
instance SumEqualWithExplanation (V level) where
    sumConstructorPair (V a1) (V a2) =
        SumConstructorSameWithArguments (EqWrap "V" a1 a2)
...
makeEq ::
    ...
    -> PureMLPattern Meta var
    -> PureMLPattern Meta var
makeEq p1 p2 = give mockMetadataTools (mkEquals p1 p2)
...
makeAnd ::
    ...
    -> PureMLPattern Meta var
    -> PureMLPattern Meta var
makeAnd p1 p2 = give mockMetadataTools (mkAnd p1 p2)
...
makeEquals ::
    ...
    -> PureMLPattern Meta var
    -> Predicate Meta var
makeEquals p1 p2 = give mockMetadataTools (makeEqualsPredicate p1 p2)
