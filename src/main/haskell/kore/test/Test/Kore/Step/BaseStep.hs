...
import Kore.Step.ExpandedPattern (CommonExpandedPattern)
import Kore.Unification.Error
import Kore.Unification.Unifier (FunctionalProof(..), UnificationProof(..))
...
test_baseStep =
    ...
    , testCase
          "Substituting a variable with a larger one."
          (assertEqualWithExplanation
               ""
               (Right
                    ( ExpandedPattern
    ...
    , testCase
          "Substituting a variable with itself."
          (assertEqualWithExplanation
               ""
               (Right
                    ( ExpandedPattern
    ...
    , testCase
          "Merging configuration patterns."
          (assertEqualWithExplanation
               ""
               (Right
                    ( ExpandedPattern
    ...
    -- Expected: y
    , testCase
          "Identical variables have no condition, alphabetical larger."
          (identicalVariablesAssertion y1)
    -- sigma(x, x) => x   vs   sigma(a, a)
    -- Expected: a
    , testCase
          "Identical variables have no condition, alphabetical lower."
    ...
    , testCase
          "Rename quantified rhs variables."
          (assertEqualWithExplanation
               ""
               (Right
                    ( ExpandedPattern
    ...
    , let fOfB = metaF (b1 PatternSort)
       ...
       in testCase
              "Merging substitution with existing one."
              (assertEqualWithExplanation
                   ""
                   (Right
                        ( ExpandedPattern
       ...
       in testCase
              ...
              (assertEqualWithExplanation
                   ...
                   (Right
                        ( ExpandedPattern
                              { term = asPureMetaPattern (metaSigma fOfB fOfB)
                              , predicate = makeEquals (metaG fOfB) (metaF fOfB)
                              , substitution =
                                    [ ( asVariable (a1 PatternSort)
                                      , asPureMetaPattern fOfB)
                                    ]
  ...
  where
    ...
    identicalVariablesAssertion var =
        assertEqualWithExplanation
            ...
            (Right
                 ( ExpandedPattern
                       { term = asPureMetaPattern (var PatternSort)
                       , predicate = makeTruePredicate
                       , substitution = []
                       }
                 , StepProofCombined
            ...
            (runStep
                 ...
                 ExpandedPattern
                     { term =
                           asPureMetaPattern
                               (metaSigma (var PatternSort) (var PatternSort))
                     , predicate = makeTruePredicate
                     , substitution = []
                     }
                 AxiomPattern
...
runStep ::
    ...
    -> AxiomPattern level
    -> Either (StepError level Variable) ( CommonExpandedPattern level
                                         , StepProof level)
