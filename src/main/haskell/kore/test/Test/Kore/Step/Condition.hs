module Test.Kore.Step.Condition
    ( mockConditionEvaluator
    ) where

import Kore.Predicate.Predicate (Predicate, PredicateProof(..))
import Kore.Step.Function.Data (ConditionEvaluator(..))
...
mockConditionEvaluatorHelper [] condition = return (condition, PredicateProof)
mockConditionEvaluatorHelper ((condition, result):reminder) unevaluatedCondition =
