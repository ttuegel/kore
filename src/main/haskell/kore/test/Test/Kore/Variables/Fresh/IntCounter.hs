module Test.Kore.Variables.Fresh.IntCounter
    ( test_intCounter
    ) where
    ...
    , testCase
          ...
          ((evaluate
                (runIntCounter
                     ...
                     2) >>
            assertFailure "This evaluation should fail") `catch` \(ErrorCall s) ->
               assertEqual "" "Cannot generate variable satisfying predicate" s)
    , testCase
