...
import Data.Word
import GHC.Generics
import Prelude hiding (all, and, lines, not, pred, succ)
...
data Simple_rule
    ...
    | Rule_gen Nat
               Simple_proof
    | Rule_use_hyp Nat
    | Ax_propositional1 Formula
                        Formula
    | Ax_propositional2 Formula
                        Formula
                        Formula
    | Ax_propositional3 Formula
                        Formula
    | Ax_varSubst Nat
                  Nat
                  Formula
 ...
 -- or_intro1 proves A -> A \/ B
    | Ax_or_intro1 Formula
                   Formula
 -- or_intro1 proves B -> A \/ B
    | Ax_or_intro2 Formula
                   Formula
...
pFormula = atoms <|> parens phrases
  where
    ...
    phrases =
        Plus <$ symbol "Plus" <*> pFormula <*> pFormula <|>
...
pCtx = atoms <|> parens phrases
  where
    atoms = SuccC <$ symbol "SuccC" <|> DefinedC <$ symbol "DefinedC"
    phrases =
        PlusLC <$ symbol "PlusLC" <*> pFormula <|>
...
pSimple_rule = atoms <|> parens phrases
  where
    ...
    phrases =
        ...
        Ax_propositional1 <$ symbol "Ax_propositional1" <*> pFormula <*>
        pFormula <|>
        Ax_propositional2 <$ symbol "Ax_propositional2" <*> pFormula <*>
        pFormula <*>
        pFormula <|>
        Ax_propositional3 <$ symbol "Ax_propositional3" <*> pFormula <*>
        pFormula <|>
        Ax_varSubst <$ symbol "Ax_varSubst" <*> pNat <*> pNat <*> pFormula <*>
        pFormula <|>
        ...
        Ax_singvar <$ symbol "Ax_singvar" <*> pCtx_path <*> pCtx_path <*> pNat <*>
        pFormula <|>
...
convertRule ::
    ...
    => (Simple_proof -> m Int)
    -> Simple_rule
    -> m (Either Int (MLRule Text Text Int (AST.Pattern Text Text Int) Int))
convertRule convertHyp r =
    case r of
        Rule_mp h1 h2 ->
            Right <$> (ModusPonens <$> convertHyp h2 <*> convertHyp h1)
  -- ^ NB. argument order is flipped!
        ...
        axiom ->
            pure . Right $
            case axiom of
                Ax_propositional1 f1 f2 ->
                    Propositional1 (convertFormula f1) (convertFormula f2)
                Ax_propositional2 f1 f2 f3 ->
                    Propositional2
                        (convertFormula f1)
                        (convertFormula f2)
                        (convertFormula f3)
                Ax_propositional3 f1 f2 ->
                    Propositional3 (convertFormula f1) (convertFormula f2)
                Ax_varSubst x y p1 _ ->
                    VariableSubstitution
                        (SubstitutedVariable (nat2Int x))
                        (convertFormula p1)
                ...
                Ax_allImp x f1 f2 ->
                    ForallRule
                        ...
                        (convertFormula f2)
                Ax_out_ex ctx var p ->
                    ctxRule PropagateExists ctx (nat2Int var) (convertFormula p)
                Ax_out_or ctx f1 f2 ->
                    ctxRule
                        PropagateOr
                        ctx
                ...
                Ax_and_intro f1 f2 ->
                    AndIntro (convertFormula f1) (convertFormula f2)
                Ax_or_elim f1 f2 f3 ->
                    OrElim
                        (convertFormula f1)
                        (convertFormula f2)
                        (convertFormula f3)
                Ax_or_intro1 f1 f2 ->
                    OrIntroL (convertFormula f1) (convertFormula f2)
                Ax_or_intro2 f1 f2 ->
                    OrIntroR (convertFormula f1) (convertFormula f2)
                rule ->
                    error $ "This case should be handled earlier: " ++ show rule
...
{- The ConvM is for building a proof
 ...
 -}
newtype ConvM a = ConvM
    { runConvM :: State (Int, [(Int, TextPat, TextRule Int)]) a
...
emit pat rule =
    ConvM
        (state (\(next, lines) -> (next, (next + 1, (next, pat, rule) : lines))))

convert :: Simple_proof -> ConvM Int
convert (Conclusion formula rule) =
    convertRule convert rule >>= \case
        Right rule' -> emit (convertFormula formula) rule'
        Left ix -> return ix

useHyp :: (Applicative f) => (Nat -> f Nat) -> (Simple_proof -> f Simple_proof)
useHyp f (Conclusion pat (Rule_use_hyp v)) =
    Conclusion pat . Rule_use_hyp <$> f v
...
checkPattern ::
       (ReifiesSignature s)
    => TextPat
    -> Maybe (AST.WFPattern (SimpleSignature s) Int)
  ...
  where
    ...
    go depth parent siblings ((grandparent, siblings'):stack) (')':str') =
        go
            (depth - 1)
            grandparent
            (Tree.Node parent (reverse siblings) : siblings')
            stack
            str'
    go 0 _ [n] [] "" = n
    go depth parent siblings stack (_:str) = go depth parent siblings stack str
...
findBadThings ::
    ...
    -> [(Int, TextPat, TextRule Int)]
    -> [(Int, TextPat, TextRule Int)]
findBadThings proxy entries = [e | e <- entries, checkEntry' proxy e == Nothing]
...
startProof ::
       (ReifiesSignature s)
    => proxy (SimpleSignature s)
    -> Either String (Proof Int (MLRuleSig (SimpleSignature s) Int) (AST.WFPattern (SimpleSignature s) Int))
startProof _proxy = do
    assumptions <-
        maybe (Left "Bad pattern") Right $
        mapM
            checkPattern
            [ defined (var 0)
        -- ^ definedness
        ...
        -- ^ definition of plus zero
            , AST.impliesP
                  "Nat"
                  (plus (succ (var 0)) (var 1))
                  (succ (plus (var 0) (var 1)))
        -- ^ definition of plus succ
            ]
    foldM
        (\pf (ix, pat) ->
             either (Left . show) Right $ add (\_ -> Right ()) pf ix pat)
        emptyProof
        (zip [0 ..] assumptions)
  where
...
runEntry ::
       (ReifiesSignature s)
    => Proof Int (MLRuleSig (SimpleSignature s) Int) (AST.WFPattern (SimpleSignature s) Int)
    -> ( Int
       , AST.WFPattern (SimpleSignature s) Int
       , MLRuleSig (SimpleSignature s) Int Int)
    -> Either String (Proof Int (MLRuleSig (SimpleSignature s) Int) (AST.WFPattern (SimpleSignature s) Int))
runEntry proof entry@(ix, pat, rule) =
    presentError $ do
...
withProof ::
       (forall (s :: *). (ReifiesSignature s) =>
                             Either String (Proof Int (MLRuleSig (SimpleSignature s) Int) (AST.WFPattern (SimpleSignature s) Int)) -> IO r)
    -> IO r
withProof body = do
    ...
    reifySignature
        ...
        (\proxy ->
             body $ do
                 ...
                 foldM
                     (\pf ->
                          (maybe (Left "Check failed") Right . checkEntry) >=>
                          runEntry pf)
                     axioms
                     entries)
