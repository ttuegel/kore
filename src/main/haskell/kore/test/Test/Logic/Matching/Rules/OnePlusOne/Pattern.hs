...
visitPatternF ::
    ...
    -> (var -> f var')
    -> (pat -> f pat')
    -> (PatternF sort label var pat -> f (PatternF sort' label' var' pat'))
visitPatternF sort label var pat term =
    case term of
...
newtype WFPattern sig var = WFPattern
    { fromWFPattern :: SigPattern sig var
    }

deriving instance
         (Eq (Sort sig), Eq (Label sig), Eq var) => Eq (WFPattern sig var)

deriving instance
         (Show (Sort sig), Show (Label sig), Show var) =>
         Show (WFPattern sig var)

deriving instance
         (Pretty (SigPatternF sig var (SigPattern sig var))) =>
...
checkSorts1 ::
       (IsSignature sig, Eq (Sort sig))
    => SigPatternF sig var (WFPattern sig var)
    -> Maybe (WFPattern sig var)
checkSorts1 pat =
    if patOk
        then Just (WFPattern (Fix (coerce pat)))
        else Nothing
  where
    patOk =
        case pat of
...
resolvePattern ::
       forall sig var. CheckableSignature sig
    => Pattern (RawSort sig) (RawLabel sig) var
    -> Maybe (SigPattern sig var)
resolvePattern p =
    cata (\pat -> recognizeLayer pat >>= sequenceA >>= return . Fix) p
  where
...
notFree ::
       (Eq sort, Eq var, ToPattern p sort label var) => sort -> var -> p -> Bool
notFree sVar var p = go (toPattern p)
  where
    go (Variable sVar' var') = not (sVar' == sVar && var' == var)
...
pattern VariableP :: (ToPattern p sort label var) =>
        sort -> var -> p

pattern VariableP sort var <- (toPattern -> Variable sort var)

pattern ApplicationP :: (ToPattern p sort label var) =>
        label -> [p] -> p

pattern ApplicationP label args <-
        (toPattern -> Application label args)
...
pattern ForallP :: (ToPattern p sort label var) =>
        sort -> sort -> var -> p -> p
...
pattern OrP :: (ToPattern p sort label var, Eq sort) =>
        sort -> p -> p -> p

pattern OrP s p1 p2 <-
        NotP s
          (AndP ((== s) -> True) (NotP ((== s) -> True) p1)
             (NotP ((== s) -> True) p2))

orP :: FromPattern p sort label var => sort -> p -> p -> p
orP s p1 p2 = notP s (andP s (notP s p1) (notP s p2))

pattern ImpliesP :: (ToPattern p sort label var, Eq sort) =>
        sort -> p -> p -> p
...
pattern IffP :: (ToPattern p sort label var, Eq p, Eq sort) =>
        sort -> p -> p -> p

pattern IffP s p1 p2 <-
        AndP s (ImpliesP ((== s) -> True) p1 p2)
...
pattern ExistsP :: (ToPattern p sort label var, Eq sort) =>
        sort -> sort -> var -> p -> p

pattern ExistsP s sVar var p <-
        NotP s
          (ForallP ((== s) -> True) sVar var (NotP ((== s) -> True) p))
...
autoSort1 ::
       (IsSigPatternIn m p sig var) => (Sort sig -> m p -> m p) -> (m p -> m p)
...
notP' :: (IsSigPatternIn m p sig var) => m p -> m p
notP' = autoSort1 notP

forallP' :: (IsSigPatternIn m p sig var) => Sort sig -> var -> m p -> m p
forallP' sVar var = autoSort1 (\s -> forallP s sVar var)

existsP' :: (IsSigPatternIn m p sig var) => Sort sig -> var -> m p -> m p
