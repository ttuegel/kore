...
newtype SubstitutedVariable var =
    SubstitutedVariable var
    deriving (Eq, Show)

newtype SubstitutingVariable var =
    SubstitutingVariable var
    deriving (Eq, Show)
...
data MLRule sort label var term hypothesis
    ...
    | Propositional3 term
                     term
    | ModusPonens hypothesis
                  hypothesis
    | Generalization var
                     hypothesis
    | VariableSubstitution (SubstitutedVariable var)
                           term
                           (SubstitutingVariable var)
    | ForallRule var
                 term
...
transformRule ::
    ...
    -> (term -> f term')
    -> (hyp -> f hyp')
    -> (MLRule sort label var term hyp -> f (MLRule sort' label' var' term' hyp'))
transformRule _sort label var term hypothesis rule =
    case rule of
        Propositional1 t1 t2 -> Propositional1 <$> term t1 <*> term t2
        Propositional2 t1 t2 t3 ->
            Propositional2 <$> term t1 <*> term t2 <*> term t3
        ...
        VariableSubstitution (SubstitutedVariable v1) t (SubstitutingVariable v2) ->
            VariableSubstitution <$> (SubstitutedVariable <$> var v1) <*> term t <*>
...
ruleTerms ::
       forall (f :: * -> *) (sort :: *) termA termB hyp label var.
       (Applicative f)
    => (termA -> f termB)
    -> (MLRule sort label var termA hyp -> f (MLRule sort label var termB hyp))
...
substVar sort varFrom varTo pat = go pat
  where
    ...
    go (Fix (AST.Variable sortVar varVar))
        | (sortVar, varVar) == (sort, varFrom) =
            Just (Fix (AST.Variable sort varTo))
...
instance (IsSignature sig, Eq (Sort sig), Eq (Label sig), Eq var) =>
    ...
    checkDerivation conclusion rule =
        case rule & ruleTerms %~ Just of
            Propositional1 a b -> expect $ a --> b --> a
            Propositional2 a b c ->
                expect $ (a --> b --> c) --> (a --> b) --> (a --> c)
            Propositional3 a b -> expect $ (notP' a --> notP' b) --> (b --> a)
            ModusPonens a (ImpliesP _ a' b)
                | a == a' -> expect $ Just b
                    ...
                    ImpliesP s (ForallP _ _ var1 body) term2
                        ...
                        , var1 == x ->
                            if Just (AST.fromWFPattern term2) ==
                               substVar s x y (AST.fromWFPattern body)
                                then Right ()
                                else Left
                                         (Error
                                              []
                                              "right hand term does not match phi[y/x]")
                        | otherwise ->
                            Left
                                (Error
                                     []
                                     "conclusion does not agree with arguments")
                    _ -> Left (Error [] "malformed conclusion")
            ForallRule var term1 term2 ->
                case conclusion of
                    ImpliesP _ (ForallP _ sVar var1 (ImpliesP _ p1 p2)) (ImpliesP _ p3 (ForallP _ sVar1 var2 p4))
                        | var == var1
                        , var1 == var2
                        , sVar == sVar1
                        , p1 == p3
                        , p2 == p4
                        , AST.notFree sVar var1 p1 ->
                            if term1 == Just p1 && term2 == Just p2
                                then Right ()
                                else Left
                                         (Error
                                              []
                                              "conclusion does not match rule arguments")
            ...
            Framing _ _ _ -> Left (Error [] "hypothesis has wrong form")
            PropagateOr label pos phi1 phi2 -> do
                case conclusion of
                    ImpliesP _ (ApplicationP label1 args1) (OrP _ (ApplicationP label2a args2a) (ApplicationP label2b args2b))
                        | label == label1
                        , label1 == label2a
                        , label1 == label2b
                        , (before1, OrP _ term1a term1b:after1) <-
                             splitAt pos args1
                        ...
                        , term1b == term2b
                        , phi1 == Just term1a
                        , phi2 == Just term1b -> Right ()
                    _ -> Left (Error [] "not proved")
     -- ^ sigma(before ..,\phi1 \/ \phi2,.. after) <->
     --     sigma(before ..,\phi1, .. after) \/ sigma(before ..,\phi2,.. after)
            PropagateExists label pos var term ->
                case conclusion of
                    ImpliesP _ (ApplicationP label1 args1) (ExistsP _ sVar2 var2 (ApplicationP label2 args2))
                        | label == label1
                        , label1 == label2
                        ...
                        , var == var1
                        , Just term1 == term
                        , all (AST.notFree sVar1 var)
                             (take pos args1 ++ drop (pos + 1) args1) ->
                            Right ()
                    _ -> Left (Error [] "not proved")
      -- ^ sigma(before ..,Ex x. phi,.. after) <-> Ex x.sigma(before ..,phi,.. after)
            Existence var ->
     ...
     -- ^ Ex x.x
                ...
                case conclusion of
                    ...
                    _ ->
                        Left
                            (Error
                                 []
                                 "expected formula of the form (A /\\ B) -> A")
            Proj2 a b ->
                case conclusion of
                    ...
                    _ ->
                        Left
                            (Error
                                 []
                                 "expected formula of the form (A /\\ B) -> B")
            AndIntro a b ->
                case conclusion of
                    ...
                    _ ->
                        Left
                            (Error
                                 []
                                 "expected formula of the form A -> B -> A /\\ B")
            OrElim a b c ->
                case conclusion of
                    ImpliesP sVar (ImpliesP sVar1 a1 c1) (ImpliesP sVar2 (ImpliesP sVar3 b1 c2) (ImpliesP sVar4 (OrP sVar5 a2 b2) c3))
                    ...
                    _ ->
                        Left
                            (Error
                                 []
                                 "expected formula of the form (A -> C) -> (B -> C) -> (A \\/ B -> C)")
            OrIntroL a b ->
                case conclusion of
                    ...
                    _ ->
                        Left
                            (Error
                                 []
                                 "expected formula of the form A -> A \\/ B")
            OrIntroR a b ->
                case conclusion of
                    ...
                    _ ->
                        Left
                            (Error
                                 []
                                 "expected formula of the form B -> A \\/ B")
      ...
      where
        ...
        followPath (ix:path) (ApplicationP _ args)
            ...
            | otherwise =
                Left
                    (Error [] "Application does not have argument at path index")
        followPath _ _ =
            Left (Error [] "Path attempted to enter non-application term")
