...
sortActualGen x
    | isObject x =
        ...
        couple (scale (`div` 2) (sortGen x))
    | otherwise =
        SortActual <$> (testId <$> elements (map show metaSortsList)) <*>
        pure []
...
unifiedVariableGen =
    scale (`div` 2) $
    oneof
        [UnifiedObject <$> variableGen Object, UnifiedMeta <$> variableGen Meta]
             ...
             if n <= 0
                 then oneof
                          [ asKorePattern . StringLiteralPattern <$>
                            stringLiteralGen
                          , asKorePattern . CharLiteralPattern <$>
                            charLiteralGen
                          ]
                 else frequency
                          [ ( 15
                            , asKorePattern <$> patternGen korePatternGen Meta)
                          , ( 15
                            , asKorePattern <$> patternGen korePatternGen Object)
                          , ( 1
                            , asKorePattern . StringLiteralPattern <$>
                              stringLiteralGen)
                          , ( 1
                            , asKorePattern . CharLiteralPattern <$>
                              charLiteralGen)
                          , ( 1
                            , asKorePattern . DomainValuePattern <$>
                              domainValueGen Object)
                          , ( 1
                            , asKorePattern . NextPattern <$>
                              nextGen korePatternGen Object)
                          , ( 1
                            , asKorePattern . RewritesPattern <$>
...
sentenceSymbolGen ::
       MetaOrObject level => level -> Gen (SentenceSymbol level pat variable)
sentenceSymbolGen x =
...
sentenceImportGen :: Gen (SentenceImport pat variable)
sentenceImportGen =
...
sentenceSortGen ::
       MetaOrObject level => level -> Gen (SentenceSort level pat var)
...
koreSentenceGen =
    oneof
        ...
        , constructUnifiedSentence SentenceImportSentence <$> sentenceImportGen
...
definitionGen senGen =
    pure Definition <*> scale (`div` 2) attributesGen <*>
    scale (`div` 2) (modulesGen senGen)

testId :: String -> Id level
testId name = Id {getId = name, idLocation = AstLocationTest}
