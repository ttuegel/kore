...
assertSubstring message first second =
    assertBool
        (message ++
         ": '" ++ first ++ "' is not a substring of '" ++ second ++ "'")
...
data EqWrap =
    forall a. EqualWithExplanation a =>
              EqWrap String
                     a
                     a
...
compareListWithExplanation l =
  ...
  where
    compareListElement ::
           Either [String] [String] -> EqWrap -> Either [String] [String]
    compareListElement (Left diff) _ = Left ("..." : diff)
...
instance (EqualWithExplanation a1, EqualWithExplanation a2) =>
         EqualWithExplanation (a1, a2) where
...
instance ( EqualWithExplanation a1
         , EqualWithExplanation a2
         , EqualWithExplanation a3
         ) =>
         EqualWithExplanation (a1, a2, a3) where
    compareWithExplanation (a, b, c) (d, e, f) =
        case compareListWithExplanation
                 [EqWrap "" a d, EqWrap "" b e, EqWrap "" c f] of
            Just err -> Just ("(" ++ err ++ ")")
            Nothing -> Nothing
    printWithExplanation (a, b, c) =
        "(" ++
        printWithExplanation a ++
        ", " ++ printWithExplanation b ++ ", " ++ printWithExplanation c ++ ")"

instance (EqualWithExplanation a1, EqualWithExplanation a2) =>
         SumEqualWithExplanation (Either a1 a2) where
...
instance (EqualWithExplanation a1, EqualWithExplanation a2) =>
         EqualWithExplanation (Either a1 a2) where
...
instance EqualWithExplanation EWEString where
...
instance EqualWithExplanation a => EqualWithExplanation [a] where
...
instance ( Show (thing (Fix thing))
         , Show1 thing
         , EqualWithExplanation (thing (Fix thing))
         ) =>
         WrapperEqualWithExplanation (Fix thing) where
...
--}
class EqualWithExplanation struct =>
      StructEqualWithExplanation struct
    where
        ...
        | expectedConstructor /= actualConstructor =
            error
                ("Different constructor names! '" ++
                 expectedConstructor ++ "' vs '" ++ actualConstructor ++ "'")
      where
        expectedConstructor = structConstructorName expected
        actualConstructor = structConstructorName actual
    structCompareWithExplanation expected actual =
        case compareListWithExplanation (structFieldsWithNames expected actual) of
            Just err ->
                Just (structConstructorName expected ++ " {" ++ err ++ "}")
            Nothing -> Nothing

class EqualWithExplanation struct =>
      WrapperEqualWithExplanation struct
    where
    ...
    wrapperCompareWithExplanation expected actual =
        case compareListWithExplanation [wrapperField expected actual] of
            Just err ->
                Just (wrapperConstructorName expected ++ " (" ++ err ++ ")")
            Nothing -> Nothing

data SumConstructor
    = SumConstructorDifferent String
                              String
...
as instances of 'EqualWithExplanation'
--}
class EqualWithExplanation sum =>
      SumEqualWithExplanation sum
    where
