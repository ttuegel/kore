...
transformRule ::
    ...
    -> (term -> f term')
    -> (hyp -> f hyp')
    -> (MLRule label var term hyp -> f (MLRule label' var' term' hyp'))
transformRule label var term hypothesis rule =
    case rule of
...
ruleTerms ::
       (Applicative f)
    => (termA -> f termB)
    -> (MLRule label var termA hyp -> f (MLRule label var termB hyp))
    ...
    checkDerivation conclusion rule =
        case rule & ruleTerms %~ Just of
            ...
            ModusPonens _ _ ->
                Left
                    (Error
                         []
                         "hypotheses have wrong form, modus ponens requires hypotheses of the form A->B and A")
                    ...
                    ImpliesP _ (ForallP _ sVar var1 body) term2
                        ...
                        , var1 == x ->
                            if Just (fromWFPattern term2) ==
                               substVar sVar x y (fromWFPattern body)
                                then Right ()
                                else Left
                                         (Error
                                              []
      ...
      --     sigma(before ..,\phi1, .. after) \/ sigma(before ..,\phi2,.. after)
            PropagateOr label pos phi1 phi2 -> do
                case conclusion of
                    IffP _ (ApplicationP label1 args1) (OrP _ (ApplicationP label2a args2a) (ApplicationP label2b args2b))
                        | label == label1
                        , label1 == label2a
                        , label1 == label2b
      ...
      --    <-> Ex x.sigma(before ..,phi,.. after)
            PropagateExists label pos var term ->
                case conclusion of
                    IffP _ (ApplicationP label1 args1) (ExistsP _ sVar2 var2 (ApplicationP label2 args2))
                        | label == label1
                        , label1 == label2
            ...
            Singvar var termP path1 path2 ->
                case conclusion of
                    NotP _ (AndP _ term1 term2) -> do
                        ...
                        case (occ1, occ2) of
                            (AndP _ (VariableP sVar1 var1) termPa, AndP _ (VariableP sVar2 var2) (NotP _ termPb))
                                | var == var1
                                , sVar1 == sVar2
                                , var1 == var2
                                , termP == Just termPa
                            ...
                            _ ->
                                Left
                                    (Error
                                         []
                                         "subterms at given locations are not properly related")
                    _ ->
                        Left
                            (Error
                                 []
                                 "conclusion of singvar does not have expected forms")
        ...
        followPath (position:path) (ApplicationP _ args)
            ...
            | otherwise =
                Left
                    (Error
                         []
                         "Path contained position greater than number of children")
        followPath _ _ =
