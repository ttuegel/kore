...
import Data.Text.Prettyprint.Doc (Doc, Pretty(pretty), (<>), sep, tupled)
import Data.Void
...
parseMLRule pLabel pVar pTerm pIx =
    propositionalRules <|> propagationRules <|> modusPonens <|> generalization <|>
    varsubst <|>
    forall <|>
    framing <|>
    existence <|>
  ...
  where
    ...
    modusPonens = rule "mp" $ ModusPonens <$> pIx `arg` pIx
    generalization = rule "ug" $ Generalization <$> pVar `arg` pIx
    varsubst =
        rule "varsubst" $
        VariableSubstitution <$> (SubstitutedVariable <$> pVar) `arg` pTerm `arg`
        (SubstitutingVariable <$> pVar)
    forall = rule "forall" $ ForallRule <$> pVar `arg` pTerm `arg` pTerm
    framing = rule "framing" $ Framing <$> pLabel `arg` number `arg` pIx
    propagationRules = do
        void $ string "propagate-"
        propagateOr <|> propagateExists
    propagateOr =
        rule "or" $ PropagateOr <$> pLabel `arg` number `arg` pTerm `arg` pTerm
    propagateExists =
        rule "exists" $
        PropagateExists <$> pLabel `arg` number `arg` pVar `arg` pTerm
    existence = rule "exists" $ Existence <$> pVar
    singvar =
        rule "singvar" $
...
parseMLRuleSig ::
       forall sig var ix. (Pattern.IsSignature sig)
    => Parser (Sort sig)
    -> Parser (Label sig)
    -> Parser var
    -> Parser ix
    -> Parser (MLRuleSig sig var ix)
parseMLRuleSig pSort pLabel pVar pIx = parseMLRule pLabel pVar parseFormula pIx
  where
...
instance (Pretty label, Pretty var, Pretty term, Pretty hyp) =>
    ...
    pretty proofRule =
        let rule :: Text -> [Doc ann] -> Doc ann
            rule prefix args = pretty prefix <> tupled args
         in case proofRule of
                Propositional1 p1 p2 ->
                    rule "propositional1" [pretty p1, pretty p2]
                Propositional2 p1 p2 p3 ->
                    rule "propositional2" [pretty p1, pretty p2, pretty p3]
                Propositional3 p1 p2 ->
                    rule "propositional3" [pretty p1, pretty p2]
                ...
                ForallRule v p1 p2 ->
                    rule "forall" [pretty v, pretty p1, pretty p2]
                Framing lbl pos h ->
                    rule "framing" [pretty lbl, pretty pos, pretty h]
                PropagateOr lbl pos p1 p2 ->
                    rule
                        "propagate-or"
                        [pretty lbl, pretty pos, pretty p1, pretty p2]
                PropagateExists lbl pos x p ->
                    rule
                        "propagate-exists"
                        [pretty lbl, pretty pos, pretty x, pretty p]
