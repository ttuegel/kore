...
newtype ValidatedSignature =
    ValidatedSignature SignatureInfo
...
isValid sigInfo =
  ...
  where
    sortOk sort = Set.member sort (sorts sigInfo)
...
instance (Reifies s ValidatedSignature) =>
         CheckableSignature (SimpleSignature s) where
    ...
    resolveSort sortName
     ...
     =
        case Set.lookupGE sortName (sorts sig) of
            Just sort'
                | sort' == sortName -> Just (SimpleSort sort')
            _ -> Nothing
      where
        sig = fromValidated (reflect @s Proxy)
    resolveLabel labelName
     ...
     =
        case Map.lookupGE labelName (labels sig) of
            Just (label', _)
                | label' == labelName -> Just (SimpleLabel label')
            _ -> Nothing
      where
        sig = fromValidated (reflect @s Proxy)
