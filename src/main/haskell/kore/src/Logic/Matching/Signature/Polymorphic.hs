...
newtype ValidatedSignature = ValidatedSignature
    { fromValidated :: SignatureInfo
    }
...
isValid sigInfo =
    all (\(arity, result, args) -> all (sortOk arity) (result : args))
        (labels sigInfo)
  where
    sortOk arity t = sortTermValid (sortCons sigInfo) arity t

validate :: SignatureInfo -> Maybe ValidatedSignature
validate sig =
    if isValid sig
        then Just (ValidatedSignature sig)
        else Nothing
...
instantiate args (PApp con cargs) =
    RawPolySort con (map (instantiate args) cargs)
        ...
        case Map.lookup name (labels sig) of
            Just (arity, result, args)
                | arity == length sortArgs ->
                    coerce
                        ( instantiate sortArgs result
                        , map (instantiate sortArgs) args)
                | otherwise ->
                    error $
                    "Encapsulation failure, label " ++
                    show name ++
                    " applied to incorrect number " ++
                    show (length args) ++ " of sort parameters"
            Nothing ->
                error $
                "Encapsulation failure, invalid label " ++
                show name ++ " found in a reflected signature"
      where
        sig = fromValidated (reflect @s Proxy)

resolveSort1 ::
       forall s. (Reifies s ValidatedSignature)
    => Text
    -> [Sort (PolySignature s)]
    -> Maybe (Sort (PolySignature s))
...
resolveLabel1 ::
    ...
    => Text
    -> [Sort (PolySignature s)]
    -> Maybe (Label (PolySignature s))
resolveLabel1 labelName args =
    case Map.lookupGE labelName (labels sig) of
        Just (label', (arity, _, _))
            | label' == labelName
            , arity == length args -> Just (PolyLabel label' (coerce args))
        _ -> Nothing
  where
    sig = fromValidated (reflect @s Proxy)

instance (Reifies s ValidatedSignature) =>
         CheckableSignature (PolySignature s) where
    type RawLabel (PolySignature s) = RawPolyLabel
    type RawSort (PolySignature s) = RawPolySort
    resolveSort (RawPolySort name args) =
        traverse resolveSort args >>= resolveSort1 name
    resolveLabel (RawPolyLabel name args) =
        traverse resolveSort args >>= resolveLabel1 name
...
reifySignature sig f = reify sig go
  where
    go :: forall (s :: *). Reifies s ValidatedSignature
       => Proxy s
       -> a
    go _ = f @s Proxy
