...
simpleSigLabel ::
       (Reifies sig ValidatedSignature)
    => Parser Text
    -> Parser (Label (SimpleSignature sig))
...
simpleSigSort ::
       (Reifies sig ValidatedSignature)
    => Parser Text
    -> Parser (Sort (SimpleSignature sig))
  ...
  where
    pPat pat =
        case pat of
            ...
            And s p1 p2 ->
                pretty ("\\and" :: Text) <> Doc.braces (pSort s) <>
                Doc.parens (pChild p1 <> Doc.comma <> pChild p2)
            Not s p ->
                pretty ("\\not" :: Text) <> Doc.braces (pSort s) <>
                Doc.parens (pChild p)
            Exists s _sVar v p ->
                pretty ("\\exists" :: Text) <> Doc.braces (pSort s) <>
                Doc.parens
                    (pVar v <> Doc.colon <> pSort s <> Doc.comma <> pChild p)
