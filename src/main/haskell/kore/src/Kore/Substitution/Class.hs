...
import Kore.ASTTraversals (patternTopDownVisitorM)
...
instance (SubstitutionClass s var pat) =>
         MapClass (SubstitutionAndQuantifiedVars s) var pat where
    isEmpty = isEmpty . substitution
    empty =
        SubstitutionAndQuantifiedVars
            {substitution = empty, quantifiedVars = Set.empty}
...
instance (SubstitutionClass s var pat) =>
         SubstitutionClass (SubstitutionAndQuantifiedVars s) var pat where
...
class ( UnifiedPatternInterface pat
    ...
    where
    substitute ::
           Fix (pat var) -> s (Unified var) (Fix (pat var)) -> m (Fix (pat var))
    substitute p s =
        runReaderT
            (substituteM p)
            SubstitutionAndQuantifiedVars
                {substitution = s, quantifiedVars = freeVariables p}

substituteM ::
       PatternSubstitutionClass s var pat m
    => Fix (pat var)
    -> ReaderT (SubstitutionAndQuantifiedVars s (Unified var) (Fix (pat var))) m (Fix (pat var))
...
substitutePreprocess ::
       (PatternSubstitutionClass s var pat m, MetaOrObject level)
    => Pattern level var (Fix (pat var))
    -> ReaderT (FixedSubstitutionAndQuantifiedVars s var pat) m (Either (Fix (pat var)) ( Pattern level var (Fix (pat var))
                                                                                        , ReaderT (FixedSubstitutionAndQuantifiedVars s var pat) m (Fix (pat var)) -> ReaderT (FixedSubstitutionAndQuantifiedVars s var pat) m (Fix (pat var))))
substitutePreprocess p = do
    s <- ask
    if isEmpty s
        then return $ Left (Fix (unifyPattern p))
        else case p of
