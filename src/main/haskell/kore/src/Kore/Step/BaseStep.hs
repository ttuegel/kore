...
import Kore.AST.PureML (CommonPurePattern, PureMLPattern, mapPatternVariables)
...
import Kore.Step.Condition.Condition (ConditionSort(..))
import Kore.Step.Error
import Kore.Step.ExpandedPattern (ExpandedPattern(ExpandedPattern))
import qualified Kore.Step.ExpandedPattern as ExpandedPattern
import Kore.Step.Substitution (mergeSubstitutions)
import Kore.Substitution.Class (Hashable(..), PatternSubstitutionClass(..))
import qualified Kore.Substitution.List as ListSubstitution
import Kore.Unification.Error (UnificationError)
import Kore.Unification.SubstitutionNormalization (normalizeSubstitution)
...
import Kore.Variables.Fresh.Class (FreshVariablesClass(freshVariableSuchThat))
import Kore.Variables.Fresh.IntCounter (IntCounter)
import Kore.Variables.Int (IntVariable(..))
...
simplifyCombinedItems = foldr (simplifyAndAdd . simplifyStepProof) []
  where
    simplifyAndAdd :: StepProof level -> [StepProof level] -> [StepProof level]
...
instance Hashable StepperVariable
...
stepWithAxiom ::
    ...
    -> AxiomPattern level
    -- ^ Rewriting axiom
    -> Either (IntCounter (StepError level Variable)) (IntCounter ( ExpandedPattern.CommonExpandedPattern level
                                                                  , StepProof level))
stepWithAxiom expandedPattern AxiomPattern { axiomPatternLeft = axiomLeftRaw
                                           , axiomPatternRight = axiomRightRaw
                                           } = do
    let wrappedExpandedPattern =
    ...
    let existingVars =
        ...
        normalizeUnificationError existingVariables action =
            stepperVariableToVariableForError
                existingVariables
                (unificationToStepError action)
        normalizeSubstitutionError action =
            stepperVariableToVariableForError
                existingVars
                (substitutionToStepError action)
    (unificationSubstitution, unificationCondition, rawSubstitutionProof) <-
        normalizeUnificationError
            existingVars
            (unificationProcedure given axiomLeft startPattern)
    (substitutionMergeCondition, substitution, _ -- TODO: Use this proof
     ) <-
        normalizeUnificationError
            existingVars
            (mergeSubstitutions unificationSubstitution startSubstitution)
    normalizedSubstitutionWithCounter <-
        normalizeSubstitutionError (normalizeSubstitution substitution)
    let (mergedConditionWithCounter, _) -- TODO: Use this proof
         =
...
mergeConditionsWithAnd conditions =
    let (predicate, proof) = makeMultipleAndPredicate conditions
     in (return predicate, proof)
...
unificationProofStepVariablesToCommon existingVars mapping (CombinedUnificationProof items) = do
    (newMapping, mappedItems) <-
        listStepVariablesToCommon
            ...
            mapping
            items
    return (newMapping, CombinedUnificationProof mappedItems)
unificationProofStepVariablesToCommon existingVars mapping (ConjunctionIdempotency patt) = do
...
unificationProofStepVariablesToCommon existingVars mapping (Proposition_5_24_3 functionalProof variable patt) = do
    ...
    (newMapping3, mappedPattern) <-
        patternStepVariablesToCommon existingVars newMapping2 patt
    return
        ( newMapping3
        , Proposition_5_24_3 mappedFunctionalProof mappedVariable mappedPattern)
unificationProofStepVariablesToCommon existingVars mapping (AndDistributionAndConstraintLifting symbolOrAlias unificationProof) = do
    (newMapping, mappedItems) <-
        listStepVariablesToCommon
            unificationProofStepVariablesToCommon
            existingVars
    ...
    return
        ( newMapping
        , AndDistributionAndConstraintLifting symbolOrAlias mappedItems)
unificationProofStepVariablesToCommon existingVars mapping (SubstitutionMerge variable patt1 patt2) = do
    (newMapping1, mappedVariable) <-
        variableStepVariablesToCommon existingVars mapping variable
    ...
    return
        ( newMapping3
        , SubstitutionMerge mappedVariable mappedPattern1 mappedPattern2)

listStepVariablesToCommon ::
       MetaOrObject level
    => (Set.Set (Variable level) -> Map.Map (StepperVariable level) (StepperVariable level) -> listElement StepperVariable -> IntCounter ( Map.Map (StepperVariable level) (StepperVariable level)
    ...
    -> IntCounter ( Map.Map (StepperVariable level) (StepperVariable level)
                  , [listElement Variable])
listStepVariablesToCommon _ _ mapping [] = return (mapping, [])
listStepVariablesToCommon elementMapper existingVars mapping (proof:proofs) = do
...
functionalProofStepVariablesToCommon existingVars mapping (FunctionalVariable variable) = do
    ...
    case variable of
        ...
        AxiomVariable av ->
            case Map.lookup variable mapping of
                Just var ->
                    case var of
                        AxiomVariable _ -> error "Unexpected axiom variable"
                        ConfigurationVariable v -> return (mapping, v)
                Nothing -> do
                    newVar <-
...
replacePatternVariables mapping =
    mapPatternVariables (\var -> fromMaybe var (Map.lookup var mapping))
...
addAxiomVariablesAsConfig existingVars mapping (ConfigurationVariable _:vars) =
    addAxiomVariablesAsConfig existingVars mapping vars
addAxiomVariablesAsConfig existingVars mapping (var@(AxiomVariable av):vars) =
    case Map.lookup var mapping of
        Just _ -> addAxiomVariablesAsConfig existingVars mapping vars
        Nothing -> do
            newVar <-
                freshVariableSuchThat av (not . (`Set.member` existingVars))
            addAxiomVariablesAsConfig
                existingVars
...
makeUnifiedSubstitution = map (Arrow.first asUnified)
