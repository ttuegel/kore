...
koreIndexedModuleToAxiomPatterns level idxMod =
    rights $
    map (koreSentenceToAxiomPattern level) (indexedModuleRawSentences idxMod)
...
patternToAxiomPattern pat =
    case project pat of
        AndPattern And { andFirst = Fix (TopPattern _)
                       , andSecond = Fix (AndPattern And { andFirst = Fix (TopPattern _)
