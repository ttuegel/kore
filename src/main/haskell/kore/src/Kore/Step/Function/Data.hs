...
import Kore.AST.Common (Application, Variable)
...
import Kore.Predicate.Predicate (Predicate, PredicateProof)
...
--}
data FunctionResultProof level =
    FunctionResultProof
    deriving (Show, Eq)
...
newtype PureMLPatternFunctionEvaluator level variable =
    PureMLPatternFunctionEvaluator (PureMLPattern level variable -> IntCounter ( ExpandedPattern level variable
...
newtype ApplicationFunctionEvaluator level variable =
    ApplicationFunctionEvaluator (forall . ( MetaOrObject level
                                           , Given (MetadataTools level)
                                           ) =>
                                               ConditionEvaluator level variable -> PureMLPatternFunctionEvaluator level variable -> Application level (PureMLPattern level variable) -> IntCounter ( AttemptedFunction level variable
                                                                                                                                                                                                    , FunctionResultProof level))
...
--}
newtype ConditionEvaluator level variable =
    ConditionEvaluator (Predicate level variable -> IntCounter ( Predicate level variable
