...
import Kore.AST.PureML (PureMLPattern, UnFixedPureMLPattern, asPurePattern)
...
import Kore.Unification.Unifier (UnificationSubstitution)
  ...
  where
    conditionEvaluator =
        evaluateFunctionCondition
            (PureMLPatternFunctionEvaluator functionEvaluator)
    functionEvaluator = evaluateFunctions functionIdToEvaluator
...
newtype FilterWrapper variable level = FilterWrapper
    { filterUnwrap :: Either (IntCounter ( ExpandedPattern level variable
                                         , FunctionResultProof level)) (UnFixedPureMLPattern level variable)
    }
...
filterUnhandledPatterns ::
    ...
    -> Either (IntCounter ( ExpandedPattern level variable
                          , FunctionResultProof level)) (UnFixedPureMLPattern level variable)
...
newtype EvaluationWrapper variable level = EvaluationWrapper
    { evaluationUnwrap :: IntCounter ( ExpandedPattern level variable
...
evaluateLocalFunction ::
    ...
    -> Map.Map (Id level) [ApplicationFunctionEvaluator level variable]
    -> Pattern level variable (IntCounter ( ExpandedPattern level variable
                                          , FunctionResultProof level))
    -> IntCounter (ExpandedPattern level variable, FunctionResultProof level)
evaluateLocalFunction conditionEvaluator functionEvaluator symbolIdToEvaluators pattIF = do
    pattF <- sequenceA pattIF
    let (mergedCondition, childrenSubstitution, _)
        ...
        unchanged = returnUnchanged mergedCondition childrenSubstitution
        patternLeveledFunction =
            PatternLeveledFunction
                ...
                , charLeveledFunction =
                      assertTrue mergedCondition .
                      assertEmpty childrenSubstitution .
                      returnUnchanged makeTruePredicate [] . CharLiteralPattern
                , applicationLeveledFunction =
                      evaluateApplication
                          conditionEvaluator
                          functionEvaluator
    ...
    evaluationUnwrap
        (applyPatternLeveledFunction patternLeveledFunction normalPattern)
  where
    returnUnchanged ::
...
evaluateApplication ::
    ...
    -> EvaluationWrapper variable level
evaluateApplication conditionEvaluator functionEvaluator symbolIdToEvaluator childrenCondition childrenSubstitution app@Application {applicationSymbolOrAlias = SymbolOrAlias
    ...
    case Map.lookup symbolId symbolIdToEvaluator of
        ...
        Just evaluators -> do
            ...
            case nub (filter notBottom mergedResults) of
                [] -> return bottom'
                [(AttemptedFunction.NotApplicable, _)] -> return unchanged
  ...
  where
    ...
    applyEvaluator app' (ApplicationFunctionEvaluator evaluator) =
        evaluator conditionEvaluator functionEvaluator app'
    notBottom =
        \case
            (AttemptedFunction.Applied ExpandedPattern {predicate = PredicateFalse}, _) ->
                False
            _ -> True
...
mergeWithConditionAndSubstitution (ConditionEvaluator conditionEvaluator) conditionToMerge substitutionToMerge (AttemptedFunction.Applied functionResult, _) =
    let (mergedCondition, mergedSubstitution, _) =
     ...
     in do evaluatedCondition <- conditionEvaluator mergedCondition
