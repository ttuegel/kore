...
substitutionToPredicate =
    foldl'
        (\predicate subst ->
             fst $
             makeAndPredicate predicate (singleSubstitutionToPredicate subst))
        makeTruePredicate
...
singleSubstitutionToPredicate ::
    ...
    => (variable level, PureMLPattern level variable)
    -> Predicate level variable
singleSubstitutionToPredicate (var, patt) = makeEqualsPredicate (mkVar var) patt
...
mergeSubstitutions ::
    ...
    -> UnificationSubstitution level variable
    -> Either (UnificationError level) ( Predicate level variable
                                       , UnificationSubstitution level variable
...
mergePredicatesAndSubstitutions predicates substitutions =
    let (substitutionMergePredicate, mergedSubstitution) =
            foldl' mergeSubstitutionWithPredicate (predicates, []) substitutions
     in ( fst $ makeMultipleAndPredicate substitutionMergePredicate
        , mergedSubstitution
        , PredicateProof)
...
mergeSubstitutionWithPredicate ::
    ...
    -> UnificationSubstitution level variable
    -> ([Predicate level variable], UnificationSubstitution level variable)
mergeSubstitutionWithPredicate (predicates, subst1) subst2 =
    case mergeSubstitutions subst1 subst2 of
        Left _ -> (makeFalsePredicate : predicates, [])
        Right (predicate, subst, _) -> (predicate : predicates, subst)
