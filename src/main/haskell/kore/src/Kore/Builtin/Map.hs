...
unify
  ...
  where
    ...
    unifyConcrete resultSort map1 map2 = do
        ...
        let
            predicatedIntersect :: Predicated level variable (Map k p)
            predicatedIntersect = (propagatePredicates . discardProofs) intersect
            ...
            result
                ...
                | not (Map.null remainder2) =
                    ...
                    ExpandedPattern.bottom
                | otherwise =
                    asBuiltinMap <$> predicatedIntersect
              where
    ...
    unifyFramed1 resultSort map1 concat' map2 x = do  -- Result monad
        ...
        remainder1 <- unifyChildren x $ asBuiltinMap (Map.difference map1 map2)
        let
            predicatedIntersect :: Predicated level variable (Map k p)
            predicatedIntersect =
                (propagatePredicates . discardProofs) intersect

            (predicatedRemainder1, _) = remainder1
