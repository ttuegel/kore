...
binaryOperator
  ...
  where
    get = liftParser ctx . parseDomainValue parser
...
{- | Evaluation failure due to an unparseable domain value.

  Such a failure indicates a bug in the well-formedness checker.

 -}
liftParser :: HasCallStack => String -> Either (Error e) a -> a
liftParser ctx result =
    case result of
