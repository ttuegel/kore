...
flexibleSort :: MetaOrObject level => Sort level
flexibleSort =
    SortVariableSort $
    SortVariable
...
pattern Bottom_ :: Sort level -> PureMLPattern level var
...
pattern DV_ ::
        Sort Object -> PureMLPattern Object var -> PureMLPattern Object var
...
pattern Exists_ ::
        Sort level ->
          var level -> PureMLPattern level var -> PureMLPattern level var
...
pattern Rewrites_ ::
        Sort Object ->
          PureMLPattern Object var ->
            PureMLPattern Object var -> PureMLPattern Object var

pattern Top_ :: Sort level -> PureMLPattern level var

pattern Var_ :: var level -> PureMLPattern level var

pattern StringLiteral_ :: StringLiteral -> PureMLPattern Meta var

pattern CharLiteral_ :: CharLiteral -> PureMLPattern Meta var

-- No way to make multiline pragma?
-- NOTE: If you add a case to the AST type, add another synonym here.
{-# COMPLETE And_, App_, Bottom_, Ceil_, DV_, Equals_, Exists_,
  Floor_, Forall_, Iff_, Implies_, In_, Next_, Not_, Or_, Rewrites_,
  Top_, Var_, StringLiteral_, CharLiteral_ #-}
...
patternLens i -- input sort
 ...
 =
  ...
  -- Rewrites_  s2   a b -> Rewrites_ <$>          o s2           <*> c a <*> c b
        Fix (RewritesPattern (Rewrites s2 a b)) ->
            Rewrites_ <$> o s2 <*> c a <*> c b
        Top_ s2 -> Top_ <$> o s2
        Var_ v -> Var_ <$> var v
...
isRigid :: MetaOrObject level => PureMLPattern level var -> Bool
isRigid p =
    case project p of
...
-- \and, \or, \implies, etc.
isFlexible :: MetaOrObject level => PureMLPattern level var -> Bool
isFlexible p =
    case project p of
...
ensureSortAgreement p =
    case makeSortsAgree $ p ^. partsOf allChildren of
        ...
        Just children ->
            p & (partsOf allChildren) .~ children & inputSort .~ childSort &
            resultSort .~
            (if isFlexible p
                 then flexibleSort
                 else childSort)
...
mkBottom :: MetaOrObject level => PureMLPattern level var
...
mkTop :: MetaOrObject level => PureMLPattern level var
...
fixmeSort :: MetaOrObject level => Sort level
fixmeSort =
    SortVariableSort
        SortVariable
