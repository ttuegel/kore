...
import Kore.Predicate.Predicate (Predicate, makeTruePredicate)
...
mapSubstitutionVariables variableMapper = map (mapVariable variableMapper)
  where
    mapVariable ::
           (variableFrom level -> variableTo level)
        -> (variableFrom level, PureMLPattern level variableFrom)
        -> (variableTo level, PureMLPattern level variableTo)
    mapVariable mapper (variable, patt) =
...
unificationSolutionToPurePattern tools ucp =
  ...
  where
    resultSort =
        getPatternResultSort (getResultSort tools) (project unifiedTerm)
    unifiedTerm = unificationSolutionTerm ucp
    andEquals = andPat . equals
    andPat first second =
        Fix $
        AndPattern
            And {andSort = resultSort, andFirst = first, andSecond = second}
    equals (var, p) =
        Fix $
        EqualsPattern
            Equals
...
data UnificationProof level variable
    ...
    | Proposition_5_24_3 [FunctionalProof level variable]
                         (variable level)
                         (PureMLPattern level variable)
    ...
    -- |= (ϕ ∧ ϕ') = (ϕ ∧ (ϕ = ϕ'))
    | AndDistributionAndConstraintLifting (SymbolOrAlias level)
    ...
    | SubstitutionMerge (variable level)
                        (PureMLPattern level variable)
                        (PureMLPattern level variable)
...
{-# ANN simplifyUnificationProof
          ("HLint: ignore Use record patterns" :: String)
        #-}
...
simplifyUnificationProof (CombinedUnificationProof []) = EmptyUnificationProof
...
simplifyUnificationProof (AndDistributionAndConstraintLifting symbolOrAlias unificationProof) =
...
simplifyCombinedItems ::
       [UnificationProof level variable] -> [UnificationProof level variable]
simplifyCombinedItems = foldr (addContents . simplifyUnificationProof) []
  where
    addContents ::
           UnificationProof level variable
        -> [UnificationProof level variable]
...
isFunctionalPattern tools = fixBottomUpVisitorM reduceM
  where
    reduceM (VariablePattern v) = Right [FunctionalVariable v]
...
simplifyAnds ::
    ...
    => MetadataTools level
    -> [PureMLPattern level variable]
    -> Either (UnificationError level) ( UnificationSolution level variable
                                       , UnificationProof level variable)
...
simplifyAnds tools (p:ps) =
    foldM
        simplifyAnds'
        ( UnificationSolution
              {unificationSolutionTerm = p, unificationSolutionConstraints = []}
        , EmptyUnificationProof)
        ps
  where
...
simplifyAnd ::
    ...
    -> Either (UnificationError level) ( UnificationSolution level variable
                                       , UnificationProof level variable)
simplifyAnd tools = fixTopDownVisitor (preTransform tools) postTransform
...
preTransform ::
    ...
    -> Either (Either (UnificationError level) ( UnificationSolution level variable
                                               , UnificationProof level variable)) (UnFixedPureMLPattern level variable)
        ...
        then Left $
             Right
                 ...
                 (ApplicationPattern ap1, ApplicationPattern ap2) ->
                     let head1 = applicationSymbolOrAlias ap1
                         head2 = applicationSymbolOrAlias ap2
                      in if isConstructor tools head1
                             then if head1 == head2
                                      then Right $
                                           ApplicationPattern
                                               Application
...
mlProposition_5_24_3 ::
    ...
    -> Either (UnificationError level) ( UnificationSolution level variable
                                       , UnificationProof level variable)
mlProposition_5_24_3 tools v functionalPattern =
    case isFunctionalPattern tools functionalPattern of
        Right functionalProof ->
            Right --Matching Logic 5.24 (3)
                ( UnificationSolution
...
postTransform ::
       Pattern level variable (Either (UnificationError level) ( UnificationSolution level variable
                                                               , UnificationProof level variable))
    -> Either (UnificationError level) ( UnificationSolution level variable
                                       , UnificationProof level variable)
postTransform (ApplicationPattern ap) = do
    ...
    return
        ( UnificationSolution
              ...
              , unificationSolutionConstraints =
                    concatMap unificationSolutionConstraints subSolutions
              }
        , AndDistributionAndConstraintLifting
...
solveGroupedSubstitution ::
    ...
    => MetadataTools level
    -> UnificationSubstitution level variable
    -> Either (UnificationError level) ( UnificationSubstitution level variable
                                       , UnificationProof level variable)
    ...
    if null nonSingletonSubstitutions
        then return (subst, EmptyUnificationProof)
            ...
            (finalSubst, proof) <-
                normalizeSubstitutionDuplication
                    tools
                    (concat singletonSubstitutions ++ subst')
            return (finalSubst, CombinedUnificationProof [proof', proof])
  where
...
unificationProcedure ::
    ...
    -> Either
        ...
        (UnificationError level) ( UnificationSubstitution level variable
                                 , Predicate level variable
                                 , UnificationProof level variable)
unificationProcedure tools p1 p2
    | p1Sort /= p2Sort = Left (SortClash p1Sort p2Sort)
    | otherwise = do
        (solution, proof) <- simplifyAnd tools conjunct
        (normSubst, normProof) <-
            normalizeSubstitutionDuplication
  ...
  where
    ...
    conjunct =
        Fix $ AndPattern And {andSort = p1Sort, andFirst = p1, andSecond = p2}
