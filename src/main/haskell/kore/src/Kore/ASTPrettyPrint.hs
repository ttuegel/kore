...
writeTwoFieldStruct flags name contenta contentb =
    writeOneFieldStructK
        flags
        name
        (prettyPrint NeedsParentheses contenta >> write " " >>
...
writeOneFieldStructK :: (PrinterOutput w m) => Flags -> String -> m () -> m ()
...
writeAttributesField :: (PrinterOutput w m) => String -> Attributes -> m ()
...
instance (PrettyPrint (a Meta), PrettyPrint (a Object)) =>
         PrettyPrint (Unified a) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (And level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Application level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Ceil level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Equals level child) where
...
instance (PrettyPrint child, PrettyPrint (variable level), MetaOrObject level) =>
         PrettyPrint (Exists level variable child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Floor level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Iff level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Implies level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (In level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Next level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Not level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Or level child) where
...
instance (PrettyPrint child, MetaOrObject level) =>
         PrettyPrint (Rewrites level child) where
...
instance (PrettyPrint child, PrettyPrint (variable level), MetaOrObject level) =>
         PrettyPrint (Pattern level variable child) where
...
instance (MetaOrObject level, PrettyPrint (variable level)) =>
         PrettyPrint (PureMLPattern level variable) where
...
instance ( MetaOrObject level
         ...
         PrettyPrint (SentenceAlias level pat variable) where
...
instance (MetaOrObject level, PrettyPrint (Fix (pat variable))) =>
         PrettyPrint (SentenceSymbol level pat variable) where
...
instance (PrettyPrint (Fix (pat variable))) =>
         PrettyPrint (SentenceImport pat variable) where
...
instance (PrettyPrint sortParam, PrettyPrint (Fix (pat variable))) =>
         PrettyPrint (SentenceAxiom sortParam pat variable) where
...
instance (MetaOrObject level, PrettyPrint (Fix (pat variable))) =>
         PrettyPrint (SentenceSort level pat variable) where
...
instance (MetaOrObject level, PrettyPrint (Fix (pat variable))) =>
         PrettyPrint (SentenceHook level pat variable) where
...
instance ( MetaOrObject level
         ...
         PrettyPrint (Sentence level sortParam pat variable) where
...
instance ( PrettyPrint sortParam
         , PrettyPrint (Fix (pat variable))
         , PrettyPrint (variable Object)
         , PrettyPrint (variable Meta)
         ) =>
         PrettyPrint (UnifiedSentence sortParam pat variable) where
...
instance ( PrettyPrint (sentence sortParam pat variable)
         , PrettyPrint sortParam
         , PrettyPrint (Fix (pat variable))
         ) =>
         PrettyPrint (Module sentence sortParam pat variable) where
...
instance ( PrettyPrint (sentence sortParam pat variable)
         ...
         PrettyPrint (Definition sentence sortParam pat variable) where
...
instance (PrettyPrint a, PrettyPrint b) => PrettyPrint (Either a b) where
    prettyPrint flags (Left x) = writeOneFieldStruct flags "Left" x
    prettyPrint flags (Right x) = writeOneFieldStruct flags "Right" x
...
instance (MetaOrObject level, PrettyPrint (variable level)) =>
         PrettyPrint (UnificationSolution level variable) where
...
instance (MetaOrObject level, PrettyPrint (variable level)) =>
         PrettyPrint (UnificationProof level variable) where
...
instance (MetaOrObject level, PrettyPrint (variable level)) =>
         PrettyPrint (FunctionalProof level variable) where
