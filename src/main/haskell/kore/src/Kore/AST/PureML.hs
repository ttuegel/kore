...
type PureSentenceAlias level = SentenceAlias level (Pattern level) Variable

-- |'PureSentenceSymbol' is the pure (fixed-@level@) version of 'SentenceSymbol'
type PureSentenceSymbol level = SentenceSymbol level (Pattern level) Variable

-- |'PureSentenceImport' is the pure (fixed-@level@) version of 'SentenceImport'
type PureSentenceImport level = SentenceImport (Pattern level) Variable
...
type PureDefinition level
     = Definition (Sentence level) (SortVariable level) (Pattern level) Variable
...
groundHead ctor location =
    SymbolOrAlias
        { symbolOrAliasConstructor = Id {getId = ctor, idLocation = location}
        , symbolOrAliasParams = []
        }
...
groundSymbol ctor = Symbol {symbolConstructor = ctor, symbolParams = []}
...
constant :: SymbolOrAlias level -> Pattern level variable child
...
type CommonPurePatternStub level = PurePatternStub level Variable
...
mapPatternVariable _ (AndPattern (And a b c)) = AndPattern (And a b c)
mapPatternVariable _ (ApplicationPattern (Application a b)) =
    ApplicationPattern (Application a b)
mapPatternVariable _ (BottomPattern (Bottom a)) = BottomPattern (Bottom a)
mapPatternVariable _ (CeilPattern (Ceil a b c)) = CeilPattern (Ceil a b c)
...
mapPatternVariable wrapper (ExistsPattern exists) =
    ExistsPattern exists {existsVariable = wrapper (existsVariable exists)}
mapPatternVariable _ (FloorPattern (Floor a b c)) = FloorPattern (Floor a b c)
mapPatternVariable wrapper (ForallPattern forall) =
    ForallPattern forall {forallVariable = wrapper (forallVariable forall)}
mapPatternVariable _ (IffPattern (Iff a b c)) = IffPattern (Iff a b c)
mapPatternVariable _ (ImpliesPattern (Implies a b c)) =
    ImpliesPattern (Implies a b c)
mapPatternVariable _ (InPattern (In a b c d)) = InPattern (In a b c d)
mapPatternVariable _ (NextPattern (Next a b)) = NextPattern (Next a b)
mapPatternVariable _ (NotPattern (Not a b)) = NotPattern (Not a b)
mapPatternVariable _ (OrPattern (Or a b c)) = OrPattern (Or a b c)
...
mapPatternVariable _ (TopPattern (Top a)) = TopPattern (Top a)
