                      ...
                      Application
                          { applicationSymbolOrAlias =
                                getSentenceSymbolOrAliasHead
                                    ...
                                    sortParameters
                          , applicationChildren =
                                fillCheckSorts argumentSorts patterns
                          }
...
sort_ sortType =
    SortActualSort
        SortActual
            { sortActualName =
                  Id {getId = show sortType, idLocation = AstLocationImplicit}
            , sortActualSorts = []
            }
...
parameterizedSymbol_ name location parameters operandSorts resultSort =
    SentenceSymbol
        { sentenceSymbolSymbol =
              Symbol
                  { symbolConstructor = Id {getId = name, idLocation = location}
...
parameterizedAlias_ name location parameters operandSorts resultSort leftPat rightPat =
    SentenceAlias
        { sentenceAliasAlias =
              Alias
                  { aliasConstructor = Id {getId = name, idLocation = location}
...
-- operands.
or_ :: CommonPurePatternStub level
    -> CommonPurePatternStub level
    -> CommonPurePatternStub level
or_ =
    binaryPattern
...
not_ =
    unaryPattern
        (\sortS pattern1 ->
             NotPattern Not {notSort = sortS, notChild = pattern1})
...
next_ =
    unaryPattern
        (\sortS pattern1 ->
             NextPattern Next {nextSort = sortS, nextChild = pattern1})
...
parameterizedAxiom_ parameters (SortedPatternStub SortedPattern { sortedPatternPattern = p
                                                                , sortedPatternSort = s
                                                                }) =
    SentenceAxiom
