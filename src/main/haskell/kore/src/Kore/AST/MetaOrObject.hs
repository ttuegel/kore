...
class (Show level, Ord level, Eq level) =>
    ...
    isObject :: level -> Bool
    isObject l =
        case isMetaOrObject (toProxy l) of
            IsObject -> True
            _ -> False
    isMeta :: level -> Bool
    isMeta l =
...
instance (Pretty (thing Object), Pretty (thing Meta)) =>
         Pretty (Unified thing) where
...
-}
applyUnified :: (thing Meta -> b) -> (thing Object -> b) -> (Unified thing -> b)
...
-}
asUnified :: MetaOrObject level => thing level -> Unified thing
asUnified x =
    case isMetaOrObject x of
