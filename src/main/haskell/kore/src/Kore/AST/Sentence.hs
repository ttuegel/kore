...
newtype Attributes = Attributes
    { getAttributes :: [CommonKorePattern]
    }

deriving instance (Eq CommonKorePattern) => Eq Attributes

deriving instance (Show CommonKorePattern) => Show Attributes
...
deriving instance
         (Eq1 (pat variable), Eq (variable level)) =>
         Eq (SentenceAlias level pat variable)

deriving instance
         (Show1 (pat variable), Show (variable level)) =>
...
instance Pretty (Fix (pat variable)) =>
    ...
    pretty SentenceSymbol {..} =
        Pretty.fillSep
            [ "symbol"
            , pretty sentenceSymbolSymbol <>
              Pretty.arguments sentenceSymbolSorts
            , ":"
...
instance Pretty (Fix (pat variable)) =>
    ...
    pretty SentenceImport {..} =
        Pretty.fillSep
            [ "import"
            , pretty sentenceImportModuleName
...
instance Pretty (Fix (pat variable)) =>
    ...
    pretty SentenceSort {..} =
        Pretty.fillSep
            [ "sort"
            , pretty sentenceSortName <>
              Pretty.parameters sentenceSortParameters
...
deriving instance
         (Eq1 (pat variable), Eq sortParam) =>
         Eq (SentenceAxiom sortParam pat variable)

deriving instance
         (Show1 (pat variable), Show sortParam) =>
         Show (SentenceAxiom sortParam pat variable)

instance (Pretty param, Pretty (Fix (pat variable))) =>
         Pretty (SentenceAxiom param pat variable) where
...
instance Pretty (Fix (pat variable)) =>
         Pretty (SentenceHook level pat variable) where
...
instance (Pretty (sentence sort pat variable), Pretty (Fix (pat variable))) =>
         Pretty (Module sentence sort pat variable) where
    pretty Module {..} =
...
instance (Pretty (sentence sort pat variable), Pretty (Fix (pat variable))) =>
         Pretty (Definition sentence sort pat variable) where
    pretty Definition {..} =
        Pretty.vsep (pretty definitionAttributes : map pretty definitionModules)

class SentenceSymbolOrAlias (sentence :: * -> ((* -> *) -> * -> *) -> (* -> *) -> *)
    where
...
deriving instance
         (Eq1 (pat variable), Eq (pat variable (Fix (pat variable))),
          Eq sortParam, EqMetaOrObject variable) =>
         Eq (UnifiedSentence sortParam pat variable)

deriving instance
         (Show1 (pat variable), Show (pat variable (Fix (pat variable))),
          Show sortParam, ShowMetaOrObject variable) =>
...
instance ( Pretty sortParam
         ...
         Pretty (UnifiedSentence sortParam pat variable) where
...
instance (MetaOrObject level) =>
         AsSentence KoreSentence (KoreSentenceAlias level) where
    asSentence = constructUnifiedSentence SentenceAliasSentence

instance (MetaOrObject level) =>
         AsSentence KoreSentence (KoreSentenceSymbol level) where
