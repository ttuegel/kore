  ...
  where
    ...
    stringNameNormalizer identifier@Id {getId = i} =
        if isMeta x && (i == show StringSort)
            then identifier {getId = show (MetaListSortType CharSort)}
            else identifier
...
variableRemainderParser x identifier = do
    colonParser
    sort <- sortParser x
    return Variable {variableName = identifier, variableSort = sort}
...
koreVariableOrTermPatternParser = do
    ...
    if c == '#'
        then asKorePattern <$>
             variableOrTermPatternParser korePatternParser Meta
        else asKorePattern <$>
             variableOrTermPatternParser korePatternParser Object
...
koreMLConstructorParser = do
  ...
  where
    mlPatternParser =
        keywordBasedParsers
            (map (patternString &&& koreMLConstructorRemainderParser)
                 allPatternTypes)
    ...
    objectMlConstructorRemainderParser patternType =
        case patternType of
            ...
            RewritesPatternType ->
                RewritesPattern <$>
                binaryOperatorRemainderParser korePatternParser Object Rewrites
...
definitionParser sentenceParser =
    pure Definition <*> attributesParser <*> some (moduleParser sentenceParser)
...
importSentenceRemainderParser =
    ...
    (pure SentenceImport <*> moduleNameParser <*> attributesParser)
...
axiomSentenceRemainderParser =
    asSentence <$>
    (pure SentenceAxiom <*> inCurlyBracesListParser unifiedSortVariableParser <*>
     korePatternParser <*>
     attributesParser)
...
sortSentenceRemainderParser ::
       MetaOrObject level => level -> Parser (KoreSentenceSort level)
sortSentenceRemainderParser x =
    pure SentenceSort <*> idParser x <*>
...
hookedSymbolSentenceRemainderParser =
    ...
    SentenceHookedSymbol . unRotate31 <$>
    symbolSentenceRemainderParser
        Object
        (symbolParser Object)
        (Rotate31 <....> SentenceSymbol)
...
purePatternParser ::
       MetaOrObject level => level -> Parser (CommonPurePattern level)
