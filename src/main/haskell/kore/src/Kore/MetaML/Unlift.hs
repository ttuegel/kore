...
import Kore.AST.Sentence (SentenceSymbolOrAlias(..))
import Kore.Implicit.ImplicitKore (mlPatternP, variable)
...
unliftObjectId Id {getId = ('#':oid), idLocation = location} =
...
unliftSortConstructor SymbolOrAlias { symbolOrAliasConstructor = Id { getId = ('#':'`':name)
                                                                    , idLocation = location
                                                                    }
                                    , symbolOrAliasParams = []
...
instance UnliftableFromMetaML (SortActual Object) where
    unliftFromMeta (Fix (ApplicationPattern sa)) = do
        ...
        return
            SortActual
                {sortActualName = sortConstructor, sortActualSorts = sortParams}
    unliftFromMeta _ = Nothing
...
unliftBinaryOpPattern ::
    ...
    -> (Sort Object -> CommonKorePattern -> CommonKorePattern -> p Object CommonKorePattern)
    -> ([UnliftResult] -> Maybe (Pattern Object Variable CommonKorePattern))
...
unliftEqualsInPattern ::
       (p Object CommonKorePattern -> Pattern Object Variable CommonKorePattern)
    -> (Sort Object -> Sort Object -> CommonKorePattern -> CommonKorePattern -> p Object CommonKorePattern)
    -> ([UnliftResult] -> Maybe (Pattern Object Variable CommonKorePattern))
unliftEqualsInPattern unifiedCtor ctor [rOperandSort, rResultSort, rFirst, rSecond] =
    unifiedCtor <$>
    (pure ctor <*> unliftFromMeta (unliftResultOriginal rOperandSort) <*>
     unliftFromMeta (unliftResultOriginal rResultSort) <*>
     pure (unliftResultFinal rFirst) <*>
...
unliftQuantifiedPattern ::
       (p Object Variable CommonKorePattern -> Pattern Object Variable CommonKorePattern)
    -> (Sort Object -> Variable Object -> CommonKorePattern -> p Object Variable CommonKorePattern)
    -> ([UnliftResult] -> Maybe (Pattern Object Variable CommonKorePattern))
