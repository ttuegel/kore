...
Please refer to Section 9.2 (The Kore Language Semantics) of
...
liftReducer :: UnifiedPattern Variable CommonMetaPattern -> CommonMetaPattern
liftReducer up =
    ...
    symbolDeclaration =
        SentenceSymbol
            { sentenceSymbolSymbol =
                  groundSymbol (Id symbolId liftedSymbolLocation)
  ...
  where
    ...
    freshVariable n s =
        ...
        VariablePattern
            Variable
                { variableName =
                      Id ("#P" ++ show (n :: Int)) liftedSymbolLocation
                , variableSort = s
                }
            ...
            , sentenceAxiomPattern =
                  ...
                  Equals
                      ...
                      , equalsFirst =
                            Fix $
                            apply
                                (groundHead liftedSymbolId liftedSymbolLocation)
...
liftAliasDeclaration ::
       KoreSentenceAlias Object -> (MetaSentenceSymbol, MetaSentenceAxiom)
    ...
    [ SentenceAliasSentence
          msa
              ...
              , sentenceAliasLeftPattern =
                    fmap liftToMeta (sentenceAliasLeftPattern msa)
              , sentenceAliasRightPattern =
                    fmap liftToMeta (sentenceAliasRightPattern msa)
              }
    ]
liftMetaSentence (SentenceSymbolSentence mss) =
    [ SentenceSymbolSentence
          mss {sentenceSymbolAttributes = sentenceSymbolAttributes mss}
    ]
...
liftMetaSentence (SentenceAxiomSentence as) =
  ...
  where
    metaParameters = [sv | UnifiedMeta sv <- sentenceAxiomParameters as]
    originalPattern = sentenceAxiomPattern as
    axiomSort =
        case getUnifiedPattern (project originalPattern) of
            UnifiedObject _ -> patternMetaSort
            UnifiedMeta p ->
                getPatternResultSort undefinedHeadSort (unRotate31 p)
    objectParameters = [sv | UnifiedObject sv <- sentenceAxiomParameters as]
...
liftMetaSentence (SentenceImportSentence is) =
    [ SentenceImportSentence
          is {sentenceImportAttributes = sentenceImportAttributes is}
    ]
...
liftObjectSentence (SentenceAliasSentence osa) =
    let (mas, axiom) = liftAliasDeclaration osa
     in [SentenceSymbolSentence mas, SentenceAxiomSentence axiom]
