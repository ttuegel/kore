...
import Kore.AST.PureML (PureSentenceSymbol)
...
implicitSymbol ::
       String -> [Sort level] -> Sort level -> PureSentenceSymbol level
...
applicationP =
    implicitSymbol
        "#application"
        [symbolMetaSort, patternListMetaSort]
        patternMetaSort
...
mlPatternP DomainValuePatternType =
    implicitSymbol "#\\dv" [sortMetaSort, stringMetaSort] patternMetaSort
...
[andA, bottomA, ceilA, _, equalsA, existsA, floorA, forallA, iffA {-dvA-}
                                                             , impliesA, inA, _, notA, orA, _, topA {-nextA-}
                                                                                                {-rewritesA-}
 ] = map mlPatternA allPatternTypes

[andP, bottomP, ceilP, dvP, equalsP, existsP, floorP, forallP, iffP, impliesP, inP, nextP, notP, orP, rewritesP, topP] =
...
wellFormedPatternsAxioms =
    [ parameterizedAxiom_ [pS] (wellFormedPatternsA [spS] [nilPatternListA])
    , parameterizedEqualsAxiom
          [pS]
...
stringVariable_ name =
    Variable
        { variableName = Id {getId = name, idLocation = AstLocationImplicit}
