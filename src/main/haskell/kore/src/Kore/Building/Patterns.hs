...
import Kore.AST.Kore (CommonKorePattern, asKorePattern)
...
-}
class MetaOrObject level =>
      ProperPattern level sort patt
    | patt -> sort level
    where
    asProperPattern :: patt -> Pattern level Variable CommonKorePattern
...
class AsAst CommonKorePattern patt =>
      AsMetaPattern patt
    where
...
class AsAst CommonKorePattern patt =>
      ObjectPattern sort patt


class AsAst CommonKorePattern patt =>
      MetaPattern sort patt


-------------------------------------
instance forall level sort patt. (ProperPattern level sort patt) =>
         AsAst CommonKorePattern patt where
    asAst pat =
...
instance ( MetaOrObject level
         ...
         ProperPattern level sort (PatternAnd pattern1 pattern2 sort level) where
...
metaAnd ::
    ...
    -> pattern1
    -> pattern2
    -> MetaAnd pattern1 pattern2 sort
metaAnd = PatternAnd
...
objectAnd ::
    ...
    -> pattern1
    -> pattern2
    -> ObjectAnd pattern1 pattern2 sort
...
instance (MetaOrObject level, AsSort level sort) =>
         ProperPattern level sort (PatternBottom sort level) where
    asProperPattern (PatternBottom sort) = BottomPattern (Bottom (asAst sort))
...
instance ( MetaOrObject level
         ...
         ProperPattern level resultSort (PatternCeil childSort child resultSort level) where
    asProperPattern (PatternCeil (ResultSort resultSort) operandSort child) =
        CeilPattern
            Ceil
...
metaCeil ::
       (MetaSort resultSort, MetaSort childSort, MetaPattern childSort child)
...
instance (ObjectSort sort, MetaPattern CharListSort pattern1) =>
         ProperPattern Object sort (ObjectDomainValue pattern1 sort) where
...
objectDomainValue ::
    ...
    => sort
    -> pattern1
    -> ObjectDomainValue pattern1 sort
...
instance ( MetaOrObject level
         ...
         ProperPattern level resultSort (PatternEquals childSort pattern1 pattern2 resultSort level) where
    asProperPattern (PatternEquals (ResultSort resultSort) operandSort first second) =
        EqualsPattern
            Equals
...
instance ( MetaOrObject level
         ...
         ProperPattern level sort (PatternExists (PatternVariable sv level) pattern1 sort level) where
...
instance ( MetaOrObject level
         ...
         ProperPattern level resultSort (PatternFloor childSort child resultSort level) where
    asProperPattern (PatternFloor (ResultSort resultSort) operandSort child) =
        FloorPattern
            Floor
...
instance ( MetaOrObject level
         ...
         ProperPattern level sort (PatternForall (PatternVariable variableSort level) pattern1 sort level) where
...
instance ( MetaOrObject level
         ...
         ProperPattern level sort (PatternIff pattern1 pattern2 sort level) where
...
metaIff ::
    ...
    -> pattern1
    -> pattern2
    -> MetaIff pattern1 pattern2 sort
metaIff = PatternIff
...
objectIff ::
    ...
    -> pattern1
    -> pattern2
    -> ObjectIff pattern1 pattern2 sort
...
instance ( MetaOrObject level
         ...
         ProperPattern level sort (PatternImplies pattern1 pattern2 sort level) where
...
metaImplies ::
    ...
    -> pattern1
    -> pattern2
    -> MetaImplies pattern1 pattern2 sort
...
objectImplies ::
    ...
    -> pattern1
    -> pattern2
    -> ObjectImplies pattern1 pattern2 sort
...
instance ( MetaOrObject level
         ...
         ProperPattern level resultSort (PatternIn childSort pattern1 pattern2 resultSort level) where
    asProperPattern (PatternIn (ResultSort resultSort) operandSort (ContainedChild first) second) =
        InPattern
            In
...
instance (ObjectSort sort, ObjectPattern sort pattern1) =>
         ProperPattern Object sort (ObjectNext pattern1 sort) where
...
instance ( MetaOrObject level
         ...
         ProperPattern level sort (PatternNot pattern1 sort level) where
...
instance ( MetaOrObject level
         ...
         ProperPattern level sort (PatternOr pattern1 pattern2 sort level) where
...
objectOr ::
    ...
    -> pattern1
    -> pattern2
    -> ObjectOr pattern1 pattern2 sort
objectOr = PatternOr
...
instance ( ObjectSort sort
         , ObjectPattern sort pattern1
         , ObjectPattern sort pattern2
         ) =>
         ProperPattern Object sort (ObjectRewrites pattern1 pattern2 sort) where
...
objectRewrites ::
    ...
    -> pattern1
    -> pattern2
    -> ObjectRewrites pattern1 pattern2 sort
...
instance (MetaOrObject level, AsSort level sort) =>
    ...
    asProperPattern (PatternTop sort) = TopPattern (Top (asAst sort))
...
instance ProperPattern Meta CharListSort MetaChar where
    asProperPattern (MetaChar value) = CharLiteralPattern (CharLiteral value)
...
instance (MetaOrObject level, AsSort level sort) =>
         ProperPattern level sort (PatternVariable sort level) where
    asProperPattern var = VariablePattern (asVariable var)

asVariable ::
       (MetaOrObject level, AsSort level sort)
    => PatternVariable sort level
    -> Variable level
...
metaVariable ::
       MetaSort sort => String -> AstLocation -> sort -> MetaVariable sort
...
objectVariable ::
       ObjectSort sort => String -> AstLocation -> sort -> ObjectVariable sort
