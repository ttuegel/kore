...
emptyDeclaredVariables =
    DeclaredVariables
        {objectDeclaredVariables = Map.empty, metaDeclaredVariables = Map.empty}

data VerifyHelpers level = VerifyHelpers
    { verifyHelpersFindSort :: !(Id level -> Either (Error VerifyError) (SortDescription level))
    , verifyHelpersLookupAliasDeclaration :: !(Id level -> Either (Error VerifyError) (KoreSentenceAlias level))
    , verifyHelpersLookupSymbolDeclaration :: !(Id level -> Either (Error VerifyError) (KoreSentenceSymbol level))
    , verifyHelpersFindDeclaredVariables :: !(Id level -> Maybe (Variable level))
    }
...
metaVerifyHelpers indexedModule declaredVariables =
    VerifyHelpers
        ...
        , verifyHelpersLookupAliasDeclaration = resolveAlias indexedModule
        , verifyHelpersLookupSymbolDeclaration = resolveSymbol indexedModule
...
addDeclaredVariable (UnifiedMeta variable) variables@DeclaredVariables {metaDeclaredVariables = variablesDict} =
    variables
        { metaDeclaredVariables =
              Map.insert (variableName variable) variable variablesDict
        }
addDeclaredVariable (UnifiedObject variable) variables@DeclaredVariables {objectDeclaredVariables = variablesDict} =
    variables
...
internalVerifyPattern ::
    ...
    -> DeclaredVariables
    -> Either (Error VerifyError) VerifySuccess
internalVerifyPattern korePattern mUnifiedSort indexedModule sortParamsSet declaredVariables =
    applyKorePattern
...
internalVerifyMetaPattern ::
    ...
    -> Either (Error VerifyError) VerifySuccess
internalVerifyMetaPattern maybeExpectedSort indexedModule sortVariables declaredVariables p =
...
internalVerifyObjectPattern ::
    ...
    -> Either (Error VerifyError) VerifySuccess
internalVerifyObjectPattern maybeExpectedSort indexedModule sortVariables declaredVariables p =
    withLocationAndContext
        p
        (patternNameForContext p)
        (do sort <-
...
newtype SortOrError level = SortOrError
    { getSortOrError :: Either (Error VerifyError) (Sort level)
    }
...
verifyMLPattern ::
    ...
    -> DeclaredVariables
    -> Either (Error VerifyError) (Sort level)
verifyMLPattern mlPattern indexedModule verifyHelpers declaredSortVariables declaredVariables = do
    mapM_
...
verifyPatternsWithSorts ::
    ...
    -> DeclaredVariables
    -> Either (Error VerifyError) VerifySuccess
verifyPatternsWithSorts sorts operands indexedModule declaredSortVariables declaredVariables = do
    koreFailWhen
...
verifyApplication ::
    ...
    -> DeclaredVariables
    -> Either (Error VerifyError) (Sort level)
verifyApplication application indexedModule verifyHelpers declaredSortVariables declaredVariables = do
    applicationSorts <-
        verifySymbolOrAlias
...
verifyBinder ::
    ...
    -> DeclaredVariables
    -> Either (Error VerifyError) (Sort level)
verifyBinder binder indexedModule verifyHelpers declaredSortVariables declaredVariables = do
...
applicationSortsFromSymbolOrAliasSentence ::
    ...
    -> Set.Set UnifiedSortVariable
    -> Either (Error VerifyError) (ApplicationSorts level)
applicationSortsFromSymbolOrAliasSentence symbolOrAlias sentence verifyHelpers declaredSortVariables = do
    mapM_
        (verifySort (verifyHelpersFindSort verifyHelpers) declaredSortVariables)
...
verifySameSort ::
       Unified Sort -> Unified Sort -> Either (Error VerifyError) VerifySuccess
    ...
    koreFailWithLocationsWhen
        ...
        ((renderString . Pretty.layoutCompact)
             ("Expecting sort" <+>
              ...
              "but got" <+> Pretty.squotes (pretty actualSort) <> Pretty.dot))
    verifySuccess
    ...
    koreFailWithLocationsWhen
        ...
        ((renderString . Pretty.layoutCompact)
             ("Expecting meta sort" <+>
              Pretty.squotes (pretty expectedSort) <+>
              "but got object sort" <+>
              Pretty.squotes (pretty actualSort) <> Pretty.dot))
    verifySuccess
...
addFreeVariable vars@DeclaredVariables {metaDeclaredVariables = metaVars} (UnifiedMeta v) = do
    checkVariable v metaVars
    return vars {metaDeclaredVariables = Map.insert (variableName v) v metaVars}
addFreeVariable vars@DeclaredVariables {objectDeclaredVariables = objectVars} (UnifiedObject v) = do
