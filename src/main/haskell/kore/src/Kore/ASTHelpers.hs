...
substituteSortVariables variableToSort (SortActualSort sort@SortActual {sortActualSorts = sortList}) = do
...
wrapAndQuantify s p var =
    Fix (ForallPattern
             Forall {forallSort = s, forallVariable = var, forallChild = p})

checkUnique :: Set.Set (Variable level) -> Set.Set (Variable level)
...
checkUniqueEither ::
       [Variable level] -> Map.Map String (Variable level) -> Either String ()
