...
data IndexedModule sortParam pat variable = IndexedModule
    ...
    , indexedModuleMetaAliasSentences :: !(Map.Map (Id Meta) (SentenceAlias Meta pat variable))
    , indexedModuleObjectAliasSentences :: !(Map.Map (Id Object) (SentenceAlias Object pat variable))
    , indexedModuleMetaSymbolSentences :: !(Map.Map (Id Meta) (SentenceSymbol Meta pat variable))
    , indexedModuleObjectSymbolSentences :: !(Map.Map (Id Object) (SentenceSymbol Object pat variable))
    , indexedModuleObjectSortDescriptions :: !(Map.Map (Id Object) (SentenceSort Object pat variable))
    , indexedModuleMetaSortDescriptions :: !(Map.Map (Id Meta) (SentenceSort Meta pat variable))
    , indexedModuleAxioms :: ![SentenceAxiom sortParam pat variable]
    , indexedModuleAttributes :: !(Attributes)
    , indexedModuleImports :: ![( Attributes
                                , IndexedModule sortParam pat variable)]
    , indexedModuleHookedIdentifiers :: !(Set.Set (Id Object))
    }

deriving instance
         (Show1 (pat variable), Show (pat variable (Fix (pat variable))),
          Show sortParam, Show (variable Meta), Show (variable Object)) =>
...
indexedModuleRawSentences im =
    map asSentence (Map.elems (indexedModuleMetaAliasSentences im)) ++
    map asSentence (Map.elems (indexedModuleObjectAliasSentences im)) ++
    map asSentence (Map.elems (indexedModuleMetaSymbolSentences im)) ++
    map asSentence (Map.elems (indexedModuleMetaSortDescriptions im)) ++
    map hookSymbolIfNeeded (Map.toList (indexedModuleObjectSymbolSentences im)) ++
    map hookSortIfNeeded (Map.toList (indexedModuleObjectSortDescriptions im)) ++
  ...
  where
    ...
    hookSymbolIfNeeded :: (Id Object, KoreSentenceSymbol Object) -> KoreSentence
...
indexedModuleWithMetaSorts name =
    ...
    , Map.insert
          ...
          AstLocationImplicit
          (Map.fromList
               (map (getId &&& idLocation) (Set.toList (Map.keysSet msd)))))
  where
...
metaSortDescription sortType =
  ...
  where
    sortId = Id {getId = show sortType, idLocation = AstLocationImplicit}
...
indexImplicitModule (indexedModules, lastIndexedModule) rawModule = do
    newModules <-
        indexModuleIfNeeded lastIndexedModule Map.empty indexedModules rawModule
...
indexModuleMetaSentence ::
    ...
    -> Either (Error a) ( Map.Map ModuleName KoreIndexedModule
                        , KoreIndexedModule)
indexModuleMetaSentence _ _ _ (indexedModules, indexedModule@IndexedModule {indexedModuleMetaAliasSentences = sentences}) (SentenceAliasSentence sentence) =
    return
        ( indexedModules
        , indexedModule
...
indexModuleMetaSentence _ _ _ (indexedModules, indexedModule@IndexedModule {indexedModuleMetaSymbolSentences = sentences}) (SentenceSymbolSentence sentence) =
    return
        ( indexedModules
        , indexedModule
              { indexedModuleMetaSymbolSentences =
...
indexModuleMetaSentence _ _ _ (indexedModules, indexedModule@IndexedModule {indexedModuleAxioms = sentences}) (SentenceAxiomSentence sentence) =
    return
        ( indexedModules
        , indexedModule {indexedModuleAxioms = sentence : sentences})
indexModuleMetaSentence implicitModule importingModules nameToModule (indexedModules, indexedModule@IndexedModule {indexedModuleImports = indexedImports}) (SentenceImportSentence SentenceImport { sentenceImportModuleName = importedModuleName
...
indexModuleMetaSentence _ _ _ (indexedModules, indexedModule) (SentenceSortSentence sentence) =
    return
        ( indexedModules
        , indexedModule
...
indexModuleObjectSentence ::
    ...
    -> Either (Error a) ( Map.Map ModuleName KoreIndexedModule
                        , KoreIndexedModule)
indexModuleObjectSentence _ _ _ (indexedModules, indexedModule@IndexedModule {indexedModuleObjectAliasSentences = sentences}) (SentenceAliasSentence sentence) =
    return
        ( indexedModules
        , indexedModule
              { indexedModuleObjectAliasSentences =
...
indexModuleObjectSentence _ _ _ (indexedModules, indexedModule@IndexedModule {indexedModuleObjectSymbolSentences = sentences}) (SentenceSymbolSentence sentence) =
    return
        ( indexedModules
        , indexedModule
              { indexedModuleObjectSymbolSentences =
...
indexModuleObjectSentence implicitModule importingModules nameToModule indexedStuff (SentenceSortSentence sentence) = do
...
indexModuleObjectSentence implicitModule importingModules nameToModule indexedStuff (SentenceHookSentence (SentenceHookedSort sentence)) = do
    let sortId = sentenceSortName sentence
    (indexedModules, indexedModule) <-
        indexModuleObjectSentence
            implicitModule
    ...
    return
        ...
        , indexedModule
              { indexedModuleHookedIdentifiers =
                    Set.insert
                        sortId
                        (indexedModuleHookedIdentifiers indexedModule)
              })
indexModuleObjectSentence _ _ _ (indexedModules, indexedModule@IndexedModule { indexedModuleObjectSymbolSentences = sentences
    ...
    return
        ...
        , indexedModule
              { indexedModuleObjectSymbolSentences =
                    Map.insert symbolId sentence sentences
...
indexImportedModule ::
    ...
    -> Either (Error a) ( Map.Map ModuleName KoreIndexedModule
                        , KoreIndexedModule)
indexImportedModule implicitModule importingModules nameToModule indexedModules importedModuleName = do
    koreModule <-
